...
class PipelineConfig(BaseModel):
    ...
    # Shapefile paths
    shapefiles: dict[str, str] = Field(
        description="Paths to shapefiles for geocoding"
    )
    ...
    crs: dict[str, int] = Field(
        description="CRS EPSG codes"
    )

    # Data lake configuration
    data_lake_root: str = Field(
        description="Root path to data lake storage"
    )

    # Reference data paths
    canonical_route_crosswalk_path: str = Field(
        description="Path to canonical route crosswalk CSV"
    )
    ...
    @classmethod
    def validate_shapefile_paths(cls, v: dict[str, str], info) -> dict[str, str]:
        ...
        for name, path in v.items():
            ...
            if remote_name and path.startswith("M:"):
                ...
                unc_zip_path = Path(f"{unc_path}.zip") if not unc_path.endswith(".zip") else unc_path_obj
    ...
    def validate_crosswalk_path(cls, v: str) -> str:
        ...
        if not Path(v).exists():
            raise FileNotFoundError(f"Canonical route crosswalk not found: {v}")
        return v
    ...
    def validate_data_lake_path(cls, v: str) -> str:
        ...
        if not Path(v).exists():
            raise FileNotFoundError(f"Data lake root not found: {v}")
        return v
    ...
    def from_yaml(cls, config_path: Path | str = "config/pipeline.yaml") -> "PipelineConfig":
        ...
        Args:
            config_path: Path to YAML configuration file (can be overridden by PIPELINE_CONFIG env var)
