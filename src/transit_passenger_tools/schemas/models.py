...
class SurveyMetadata(BaseModel):
    ...
    # ========== Primary Key ==========
    survey_id: str = Field(
        ..., max_length=100, description="Unique survey identifier (operator_year format)"
    )
    ...
    inflation_year: str | None = Field(
        None, max_length=10, description="Year for inflation adjustment (nullable)"
    )

    # ========== Data Versioning and Lineage ==========
    data_version: int = Field(
        ..., ge=1, description="Version number of the data files for this survey"
    )
    data_commit: str = Field(
        ..., max_length=40, description="Git commit SHA that generated this data version"
    )
    data_hash: str = Field(
        ..., max_length=64, description="SHA256 hash of data content for deduplication"
    )
    ingestion_timestamp: datetime = Field(..., description="When this data version was ingested")
    processing_notes: str | None = Field(
        None, max_length=500, description="Notes about data processing/changes"
    )
...
class CoreSurveyResponse(BaseModel):
    ...
    # ========== Foreign Keys ==========
    survey_id: str = Field(
        ..., max_length=100, description="Foreign key to SurveyMetadata.survey_id"
    )

    # ========== Survey Identifiers ==========
    original_id: str = Field(
        ..., max_length=50, description="Original record ID from operator's survey"
    )
    ...
    # ========== Access/Egress Modes ==========
    access_mode: AccessEgressMode | None = Field(
        ..., description="Mode to access first transit encounter"
    )
    egress_mode: AccessEgressMode | None = Field(
        ..., description="Mode to egress from last transit encounter"
    )
    ...
    transfer_to: TransferOperator | None = Field(
        ..., description="Operator immediately transferring to "
    )
    ...
    first_before_operator: str | None = Field(
        ..., max_length=100, description="First operator before survey board "
    )
    first_before_operator_detail: str | None = Field(
        ..., max_length=100, description="First operator detail before survey board "
    )
    first_before_technology: str | None = Field(
        ..., max_length=50, description="First technology before survey board "
    )
    second_before_operator: str | None = Field(
        ..., max_length=100, description="Second operator before survey board "
    )
    second_before_operator_detail: str | None = Field(
        ..., max_length=100, description="Second operator detail before survey board "
    )
    second_before_technology: str | None = Field(
        ..., max_length=50, description="Second technology before survey board "
    )
    third_before_operator: str | None = Field(
        ..., max_length=100, description="Third operator before survey board "
    )
    third_before_operator_detail: str | None = Field(
        ..., max_length=100, description="Third operator detail before survey board "
    )
    third_before_technology: str | None = Field(
        ..., max_length=50, description="Third technology before survey board "
    )
    first_after_operator: str | None = Field(
        ..., max_length=100, description="First operator after survey alight "
    )
    first_after_operator_detail: str | None = Field(
        ..., max_length=100, description="First operator detail after survey alight "
    )
    first_after_technology: str | None = Field(
        ..., max_length=50, description="First technology after survey alight "
    )
    second_after_operator: str | None = Field(
        ..., max_length=100, description="Second operator after survey alight "
    )
    second_after_operator_detail: str | None = Field(
        ..., max_length=100, description="Second operator detail after survey alight "
    )
    second_after_technology: str | None = Field(
        ..., max_length=50, description="Second technology after survey alight "
    )
    third_after_operator: str | None = Field(
        ..., max_length=100, description="Third operator after survey alight "
    )
    third_after_operator_detail: str | None = Field(
        ..., max_length=100, description="Third operator detail after survey alight "
    )
    third_after_technology: str | None = Field(
        ..., max_length=50, description="Third technology after survey alight "
    )

    # ========== Technology/Path ==========
    vehicle_tech: TechnologyType | None = Field(
        None, description="Technology type of vehicle being surveyed"
    )
    first_board_tech: TechnologyType | None = Field(
        ..., description="Vehicle technology type of first boarding"
    )
    last_alight_tech: TechnologyType | None = Field(
    ...
    fare_category: FareCategory | None = Field(
    ...
    is_hispanic: bool | None = Field(
        ...,
        description="True if Hispanic/Latino or of Spanish origin, False if not, None if unknown",
    )
    ...
    eng_proficient: EnglishProficiency | None = Field(
    ...
    household_income: float | None = Field(
        ..., ge=0, description="Annual household income (continuous, nominal dollars)"
    )
    household_income_category: str | None = Field(
        None, max_length=100, description="Original household income category string"
    )
    income_approximated: bool | None = Field(
        None,
        description="True if household_income was calculated from bounds rather than provided directly",
    )

    auto_to_workers_ratio: AutoRatioCategory | None = Field(
        ..., description="Auto sufficiency (vehicle to worker ratio)"
    )
    language_at_home: str | None = Field(
        ..., max_length=100, description="Language spoken at home "
    )
    ...
    survey_type: SurveyType | None = Field(
        ..., description="Survey administration method (CATI/paper/tablet)"
    )
...
class DerivedSurveyResponse(CoreSurveyResponse):
    ...
    day_part: DayPart | None = Field(
        None, description="Part of day (EARLY AM, AM PEAK, MIDDAY, PM PEAK, EVENING)"
    )
    ...
    boardings: int | None = Field(
        ...
        ge=1,
        le=10,
        description="Number of boardings in transit trip (calculated from transfers)",
    )
    transfers_surveyed: str | None = Field(
        None, max_length=20, description="Number of transfers surveyed"
    )

    # ========== Tour Purpose Logic ==========
    tour_purp: TripPurpose | None = Field(
        None, description="Tour purpose (calculated from trip purposes + work/school status)"
    )
    tour_purp_case: str | None = Field(
        None, max_length=50, description="Tour purpose case for analysis"
    )
    at_work_prior_to_orig_purp: bool | None = Field(
        None, description="Was at work before surveyed trip"
    )
    at_work_after_dest_purp: bool | None = Field(
        None, description="Will be at work after surveyed trip"
    )
    at_school_prior_to_orig_purp: bool | None = Field(
        None, description="Was at school before surveyed trip"
    )
    at_school_after_dest_purp: bool | None = Field(
        None, description="Will be at school after surveyed trip"
    )

    # ========== Auto Sufficiency (Derived) ==========
    autos_vs_workers: str | None = Field(
        None, max_length=50, description="Auto sufficiency category (autos vs workers)"
    )
    vehicle_numeric_cat: str | None = Field(
        None, max_length=20, description="Vehicle count numeric category"
    )
    worker_numeric_cat: str | None = Field(
        None, max_length=20, description="Worker count numeric category"
    )
    ...
    commuter_rail_present: int | None = Field(
        None, ge=0, le=1, description="Commuter rail present in trip"
    )
    heavy_rail_present: int | None = Field(
        None, ge=0, le=1, description="Heavy rail present in trip"
    )
    express_bus_present: int | None = Field(
        None, ge=0, le=1, description="Express bus present in trip"
    )
    ferry_present: int | None = Field(None, ge=0, le=1, description="Ferry present in trip")
    light_rail_present: int | None = Field(
        None, ge=0, le=1, description="Light rail present in trip"
    )
    ...
    income_lower_bound: float | None = Field(
        None, description="Lower bound of income bracket (dollars) - for binned data only"
    )
    income_upper_bound: float | None = Field(
        None, description="Upper bound of income bracket (dollars) - for binned data only"
    )
    hh_income_nominal_continuous: float | None = Field(
        None, description="Household income continuous value nominal dollars"
    )
    hh_income_2023dollars_continuous: float | None = Field(
        None, description="Household income continuous value in 2023 dollars"
    )

    # ========== Demographics (Derived) ==========
    year_born_four_digit: int | None = Field(
        None, ge=1900, le=2100, description="Four-digit birth year (derived from age + survey year)"
    )

    # ========== Distance Measures ==========
    distance_orig_dest: float | None = Field(
        None, description="Distance from origin to destination (miles)"
    )
    distance_board_alight: float | None = Field(
        None, description="Distance from first board to last alight (miles)"
    )
    distance_orig_first_board: float | None = Field(
        None, description="Distance from origin to first board (miles)"
    )
    distance_orig_survey_board: float | None = Field(
        None, description="Distance from origin to survey board (miles)"
    )
    distance_survey_alight_dest: float | None = Field(
        None, description="Distance from survey alight to destination (miles)"
    )
    distance_last_alight_dest: float | None = Field(
        None, description="Distance from last alight to destination (miles)"
    )
    ...
    orig_tract_GEOID: str | None = Field(
        None, max_length=15, description="Origin census tract GEOID"
    )
    dest_tract_GEOID: str | None = Field(
        None, max_length=15, description="Destination census tract GEOID"
    )
    home_tract_GEOID: str | None = Field(None, max_length=15, description="Home census tract GEOID")
    workplace_tract_GEOID: str | None = Field(
        None, max_length=15, description="Workplace census tract GEOID"
    )
    school_tract_GEOID: str | None = Field(
        None, max_length=15, description="School census tract GEOID"
    )
    first_board_tract_GEOID: str | None = Field(
        None, max_length=15, description="First boarding location census tract GEOID"
    )
    last_alight_tract_GEOID: str | None = Field(
        None, max_length=15, description="Last alighting location census tract GEOID"
    )
    survey_board_tract_GEOID: str | None = Field(
        None, max_length=15, description="Survey boarding location census tract GEOID"
    )
    survey_alight_tract_GEOID: str | None = Field(
        None, max_length=15, description="Survey alighting location census tract GEOID"
    )

    # ========== Geographic - Census GEOIDs (County) ==========
    orig_county_GEOID: str | None = Field(None, max_length=10, description="Origin county GEOID")
    dest_county_GEOID: str | None = Field(
        None, max_length=10, description="Destination county GEOID"
    )
    home_county_GEOID: str | None = Field(None, max_length=10, description="Home county GEOID")
    workplace_county_GEOID: str | None = Field(
        None, max_length=10, description="Workplace county GEOID"
    )
    school_county_GEOID: str | None = Field(None, max_length=10, description="School county GEOID")
    first_board_county_GEOID: str | None = Field(
        None, max_length=10, description="First boarding location county GEOID"
    )
    last_alight_county_GEOID: str | None = Field(
        None, max_length=10, description="Last alighting location county GEOID"
    )
    survey_board_county_GEOID: str | None = Field(
        None, max_length=10, description="Survey boarding location county GEOID"
    )
    survey_alight_county_GEOID: str | None = Field(
        None, max_length=10, description="Survey alighting location county GEOID"
    )
    ...
    workplace_PUMA_GEOID: str | None = Field(
        None, max_length=20, description="Workplace PUMA GEOID"
    )
    school_PUMA_GEOID: str | None = Field(None, max_length=20, description="School PUMA GEOID")
    first_board_PUMA_GEOID: str | None = Field(
        None, max_length=20, description="First boarding location PUMA GEOID"
    )
    last_alight_PUMA_GEOID: str | None = Field(
        None, max_length=20, description="Last alighting location PUMA GEOID"
    )
    survey_board_PUMA_GEOID: str | None = Field(
        None, max_length=20, description="Survey boarding location PUMA GEOID"
    )
    survey_alight_PUMA_GEOID: str | None = Field(
        None, max_length=20, description="Survey alighting location PUMA GEOID"
    )

    # ========== Geographic - Resolution Level ==========
    orig_geo_level: str | None = Field(
        None, max_length=20, description="Geographic resolution level for origin"
    )
    dest_geo_level: str | None = Field(
        None, max_length=20, description="Geographic resolution level for destination"
    )
    home_geo_level: str | None = Field(
        None, max_length=20, description="Geographic resolution level for home"
    )
...
class SurveyWeight(BaseModel):
    ...
    response_id: str = Field(
        ..., max_length=100, description="Foreign key to SurveyResponse.response_id"
    )
    ...
    # ========== Weight Values ==========
    boarding_weight: float | None = Field(
