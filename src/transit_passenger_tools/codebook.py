"""Enumerated types and type-mapping constants for transit passenger survey data.
...
# Python built-in → Polars dtype (used for schema enforcement)
PYTHON_TO_POLARS: dict[type, pl.DataType] = {
    int: pl.Int64(),
    float: pl.Float64(),
    bool: pl.Boolean(),
    str: pl.Utf8(),
...
# Polars dtype → Tableau Hyper SqlType (used for Hyper export)
POLARS_TO_HYPER: dict[pl.DataType, SqlType] = {
    pl.Utf8(): SqlType.text(),
    pl.String(): SqlType.text(),
    pl.Int64(): SqlType.big_int(),
    pl.Int32(): SqlType.int(),
    pl.Int16(): SqlType.small_int(),
    pl.Int8(): SqlType.small_int(),
    pl.Float64(): SqlType.double(),
    pl.Float32(): SqlType.double(),
    pl.Boolean(): SqlType.bool(),
    pl.Date(): SqlType.date(),
    pl.Time(): SqlType.time(),
}
...
class CountWord(IntEnum):
    """Numeric equivalents for word-based count survey responses.

    Used to convert text answers (e.g. "zero", "three") for the
    *vehicles* and *workers* fields into integers.
    """
...
#: Lookup table derived from :class:`CountWord`, with an extra
#: "four or more" alias that also maps to 4.
COUNT_WORD_MAPPING: dict[str, int] = {
    m.name.lower(): m.value for m in CountWord
} | {"four or more": CountWord.FOUR}
...
class Purpose(str, Enum):
    ...
    Unified enum covering both trip-level origin/destination purposes
    (e.g. ``HOME``, ``AIRPORT``) and derived tour-level purposes
    (e.g. ``WORK_BASED``, ``AT_WORK``).
    """
