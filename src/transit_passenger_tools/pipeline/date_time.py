...
def derive_temporal_fields(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if missing:
        raise ValueError(f"date_time transform requires columns: {missing}")
    ...
    if "survey_time" in df.columns:
        ...
        df = df.with_columns([
            ...
            .then(
                pl.coalesce([
                    ...
                    pl.col("survey_time").str.strptime(pl.Time, "%I:%M:%S %p", strict=False),
        ...
        df = df.with_columns([
            pl.lit(None).cast(pl.Int32).alias("_survey_hour")
        ])
