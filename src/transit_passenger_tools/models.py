"""Pydantic data models and field dependency declarations for transit survey data.
...
class CoreSurveyResponse(BaseModel):
    ...
    # ========== Trip Purpose ==========
    orig_purp: Purpose | None = Field(description="Origin trip purpose")
    dest_purp: Purpose | None = Field(description="Destination trip purpose")
    trip_purp: Purpose | None = Field(description="Trip purpose")
...
class DerivedSurveyResponse(CoreSurveyResponse):
    ...
    # ========== Tour Purpose Logic ==========
    tour_purp: Purpose | None = Field(None, description="Tour purpose (calculated from trip purposes + work/school status)")
    ...
    # ========== Vendor Pass-Through ==========
    extra_fields: dict[str, Any] | None = Field(None, description="Vendor-specific non-standard fields preserved in data warehouse")
