...
F_INPUT_STANDARD_CSV = 'M:/Data/OnBoard/Data and Reports/_data Standardized/survey_standard_2021-04-10.csv'
F_STD_DICTIONARY_CSV = paste0('C:/Users/',Sys.getenv("USERNAME"),'/Documents/GitHub/onboard-surveys/util/standard_variable_dict.csv')

F_TM1_TAZ_CSV = 'M:/Data/GIS layers/TM1_taz/bayarea_rtaz1454_rev1_WGS84.csv'
F_SD_NAME_CSV = paste0('C:/Users/', Sys.getenv("USERNAME"), '/Documents/GitHub/bayarea_urbansim/data/superdistricts.csv')

# Output
F_COMBINED_CSV = 'M:/Data/OnBoard/Data and Reports/_data Standardized/share_data/survey_combined_2021-04-10.csv'
...
############################ add new categories and mediums into the dictionary
...
for (colname in c('orig_tm1_taz', 'dest_tm1_taz', 'home_tm1_taz', 'workplace_tm1_taz', 'school_tm1_taz')) {
  print(colname)
...
# fill NaN in taz, SD_ID with 0
for (colname in c('TM1_TAZ', 'SD_id')) {
  print(colname)
  print(nrow(all_tm1_taz[which(is.na(all_tm1_taz[colname])),]))
  all_tm1_taz[colname][is.na(all_tm1_taz[colname])] <- 0
  print(nrow(df[which(is.na(all_tm1_taz[colname])),]))
}

# fill NaN in SD_name with 'missing'
