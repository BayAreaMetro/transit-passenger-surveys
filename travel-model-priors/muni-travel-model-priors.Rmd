...
Balancing_Method <- function(select_route, select_direction, select_time_of_day, apc_df, champ_df) {
  ...
  # Fit the model
  fit_matrix <- loglin(outer(boardings_df$boardings, alightings_df$alightings) / sum(boardings_df$boardings), 
                       margin = list(1,2), 
                       start = seed_matrix, 
                       fit = TRUE, eps = THRESH, 
                       iter = ITER)$fit
...
compare_champ <- left_join(output_df, compare_champ, by = c("route", "direction", "time_of_day", "board_segment", "alight_segment"))
...
compare_apc <- left_join(three, compare_apc, by = c("route", "direction", "time_of_day", "segment")) 
