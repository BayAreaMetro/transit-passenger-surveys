...
#### _TODO_
1.  Confirm that balancing is working
2.  Catch and handle function errors
3.  Add functionality to do daily sums
...
F_OUTPUT_PRIORS = "M:/Data/OnBoard/Data and Reports/Muni/travel-model-priors/test-output-priors.csv"
F_OUTPUT_COMPS  = "M:/Data/OnBoard/Data and Reports/Muni/travel-model-priors/test-output-comparisons.csv"
```
...
#direction_vector <- c("INBOUND", "OUTBOUND")
#time_of_day_vector <- c("AM", "MD", "PM")

direction_vector <- c("INBOUND")
time_of_day_vector <- c("AM")
  ...
  for (j in 1:length(direction_vector)) {
    for (k in 1:length(time_of_day_vector)) {
      ...
      if (!(i == 1 & j == 1 & k == 1)) {
        ...
        print(paste("Route: ", route_vector[i], 
                    "  Direction: ", direction_vector[j], 
                    "  Time of Day: ", time_of_day_vector[k], sep = ""))
        
        work_df <- Balancing_Method(route_vector[i], 
                                    direction_vector[j], 
                                    time_of_day_vector[k],
                                    apc_sum, champ_sum)
...
# Boarding/alighting comparison that we will write out
one <- output_df %>%
...
```{r write}
output_df <- output_df %>%
  select(route, direction, time_of_day, board_segment, alight_segment, route)

write.csv(output_df, file = F_OUTPUT_PRIORS, row.names = FALSE, quote = F)

compare_df <- compare_df %>%
  select(route, direction, time_of_day, segment, boardings, apc_boardings, champ_boardings, 
