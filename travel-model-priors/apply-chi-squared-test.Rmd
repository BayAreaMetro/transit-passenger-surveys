...
#### _TODO_ 
1. Did I do Hellinger distance correct?
...
F_OBSERVED_FLOWS = "M:/Data/OnBoard/Data and Reports/Muni/On-to-Off/As CSV/MUNI_ON2OFF_DRAFT-FINAL_CONSOLIDATED_SUBMITTAL_6_20160513.csv"
...
hellinger_df <- hellinger_df %>%
  ...
  mutate(prob_diff = sqrt(prob_diff * prob_diff)) %>%
  group_by(route, direction, time_of_day) %>%
  summarise(hellinger_dist = sum(prob_diff)) %>%
  mutate(hellinger_dist = 1/sqrt(2) * hellinger_dist)
