...
*          See BuildDatabases.sas for script to create SAS databases.  Specific to SamTrans.
*          Work in progress.
*
* Location: M:\Data\OnBoard\Data and Reports\
*
* Author(s): dto (2013 08 21)
*
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~;
...
%let gis_geocode_weekday     = 'M:\Data\OnBoard\Data and Reports\SamTrans\Geocode\gis_geocode_weekday.csv';
%let gis_geocode_weekend     = 'M:\Data\OnBoard\Data and Reports\SamTrans\Geocode\gis_geocode_weekend.csv';
%let gis_geocode_maz_weekday = 'M:\Data\OnBoard\Data and Reports\SamTrans\Geocode\gis_geocode_weekday_maz.csv';
%let gis_geocode_maz_weekend = 'M:\Data\OnBoard\Data and Reports\SamTrans\Geocode\gis_geocode_weekend_maz.csv';

%let output_directory = M:\Data\OnBoard\Data and Reports\SamTrans\_working;
run;

* Add weekpart and combine weekend and weekday files;
* - CATI;
data day; set OnBoard.rawSamCatiWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawSamCatiWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawSamCati; set day;
run;

* - GIS;
data day; set OnBoard.rawSamGisWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawSamGisWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawSamGis; set day;
run;

* - Info;
data day; set OnBoard.rawSamInfoWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawSamInfoWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawSamInfo; set day;
run;
...
* Trip purpose (q406 is year of birth);
data catiA; set OnBoard.rawSamCati;
   orig_purp = "                         ";
   dest_purp = "                         ";
   ...
   if q1 = 6  and q406>97 then orig_purp = "grade school";
   ...
   if q2 = 6  and q406>97 then dest_purp = "grade school";
...
data catiA; set catiA;
   ...
   if q10 = 1 and q11 = 1 and q12 = 6  and q406>97 then orig_purp = "grade school";
   ...
   if q10 = 1 and q11 = 2 and q12 = 6  and q406>97 then dest_purp = "grade school";
...
data OnBoard.sam_cati_purp; set catiD;
...
data gisA;  set OnBoard.rawSamGis;
data catiA; set OnBoard.sam_cati_purp; keep survey_id q10 q11;
   rename survey_id = id; 
data geoA;  set input;
...
data OnBoard.sam_key_loc_taz; set geoE; drop _TYPE_ _FREQ_;
run;

* Set the home and work locations if they are an origin or a destionation;
data catiA; set OnBoard.sam_cati_purp; keep survey_id orig_purp dest_purp;
   rename survey_id = id;
data gisA; set OnBoard.sam_key_loc_taz;
run;
...
data OnBoard.sam_key_loc_taz; set gisC; drop orig_purp dest_purp;
run;
...
* q404 is number of workers, q405 is number of drivable vehicles;
data catiA; set OnBoard.sam_cati_purp;

proc freq data = catiA; tables q404 q404_other q405 q405_other;
run;

data catiB; set catiA;
   autoSuff = 'Missing                 ';

   if q405 = 16 then autoSuff = 'Zero autos';

   if q404 > q405 and q404<16 and q405<16 then autoSuff = 'Workers > autos';

   if q404 <= q405 and q404<16 and q405<16 then autoSuff = 'Workers <= autos';

   if q404 = 16 and q405<16 then autoSuff = 'Workers <= autos';
...
data OnBoard.sam_cati_purp_asuff; set catiB;
...
%MACRO TM_MODE(LABEL,MODE_Q,AC_SEL_Q,AC_Q,MUNI_Q,SAM_Q,VTA_Q,TRANSFER);
   ...
   * Santa Rosa City Bus;
   ...
   if &MODE_Q = 21 and &TRANSFER = 0 then transfer_from = 'SANTA ROSA CITY BUS';
   if &MODE_Q = 21 and &TRANSFER = 1 then transfer_to   = 'SANTA ROSA CITY BUS';
   ...
   * Vallejo transit;
   ...
   if &MODE_Q = 27 and &TRANSFER = 0 then transfer_from = 'VALLEJO TRANSIT';
   if &MODE_Q = 27 and &TRANSFER = 1 then transfer_to   = 'VALLEJO TRANSIT';
   * Westcat;
...
* Set the access mode;
data catiA; set OnBoard.sam_cati_purp_asuff;
...
data catiB; set catiA;
   ...
   if trim(route_surveyed) = 'KX' then survey_mode = 'express bus';
...
proc freq data = catiB; tables survey_mode;
...
data OnBoard.sam_cati_purp_asuff_path; set catiE;
...
data catiA; set OnBoard.sam_cati_purp_asuff_path; 
   ...
   if q397 = 4   then fare_medium = 'pass (paper with mag stripe)';
   if q397 = 5   then fare_medium = 'pass (day)';
   if q397 = 6   then fare_medium = 'change card';
   if q397 = 7   then fare_medium = 'token';
   if q397 = 8   then fare_medium = 'pass (annual)';
   if q397 = 9   then fare_medium = 'pass (residential)';
   if q397 = 10  then fare_medium = 'pass (inter-agency)';
   if q397 = 11  then fare_medium = 'pass (free ride)';
   if q397 = 12  then fare_medium = 'other';
   ...
   if q398 = 5 then fare_category = 'other discount';

   * Match AC Transit: NOT HISPANIC/LATINO OR OF SPANISH ORIGIN or HISPANIC/LATINO OR OF SPANISH ORIGIN;
   hispanic = 'Missing                                                    ';
   if q407 = 1 then hispanic = 'HISPANIC/LATINO OR OF SPANISH ORIGIN';
   if q407 = 2 then hispanic = 'NOT HISPANIC/LATINO OR OF SPANISH ORIGIN'; 
   
   * Match AC Transit: BLACK, WHITE, ASIAN, OTHER, MULTIRACIAL;
   race = 'Missing            ';
   if q408 = 1 then race = 'WHITE';
   if q408 = 2 then race = 'BLACK';
   if q408 = 3 then race = 'ASIAN';
   if q408 = 4 then race = 'OTHER';
   if q408 = 5 then race = 'OTHER';
   if q408 = 6 then race = 'OTHER';
   if q408 = 8 then race = 'OTHER';

   * Match AC Transit: ENGLISH ONLY, ...;
   language_at_home = 'Missing                 ';
   if q409 = 2 then language_at_home = 'ENGLISH ONLY';

   if q410 = 1  then language_at_home = 'SPANISH';
   if q410 = 2  then language_at_home = 'CHINESE-CANTONESE';
   if q410 = 3  then language_at_home = 'CHINESE-MANDARIN';
   if q410 = 4  then language_at_home = 'VIETNAMESE';
   if q410 = 5  then language_at_home = 'KOREAN';
   if q410 = 6  then language_at_home = 'TAGALOG';
   if q410 = 7  then language_at_home = 'RUSSIAN';
   if q410 = 8  then language_at_home = 'PORTUGUESE';
   if q410 = 9  then language_at_home = 'FRENCH';
   if q410 = 10 then language_at_home = 'FRENCH CREOLE';
   if q410 = 11 then language_at_home = 'POLISH';
   if q410 = 12 then language_at_home = 'OTHER'; 

   household_income = 'Missing                             ';
   if q412 = 1 then household_income = 'under $35,000';
   if q412 = 2 then household_income = '$35,000 or higher';
   if q412 = 3 then household_income = 'refused';

   if q413 = 1 then household_income = 'under $10,000';
   if q413 = 2 then household_income = '$10,000 to $25,000';
   if q413 = 3 then household_income = '$25,000 to $35,000';

   if q414 = 1 then household_income = '$35,000 to $50,000';
   if q414 = 2 then household_income = '$50,000 to $75,000';
   if q414 = 3 then household_income = '$75,000 to $100,000';
   if q414 = 4 then household_income = '$100,000 to $150,000';
   if q414 = 5 then household_income = '$150,000 or higher';

   sex = 'Missing                             '; 
   if q415 = 1 then sex = 'male';
   if q415 = 2 then sex = 'female';

   * CATI language (phone interview language);
   interview_language = 'Missing                             ';
   if q416 = 1 then interview_language = 'English';
   if q416 = 2 then interview_language = 'Spanish';
   if q416 = 3 then interview_language = 'Cantonese Chinese';
   if q416 = 4 then interview_language = 'Mandarin Chinese';
   if q416 = 5 then interview_language = 'Tagalog';

   * English proficiency;
   eng_proficient = 'Missing                             ';
   if q411 = 1 then eng_proficient = 'VERY WELL';
   if q411 = 2 then eng_proficient = 'WELL';
   if q411 = 3 then eng_proficient = 'NOT WELL';
   if q411 = 4 then eng_proficient = 'NOT AT ALL';
...
data OnBoard.sam_cati_purp_asuff_path_sd; set catiA;
...
data catiA; set OnBoard.sam_cati_purp_asuff_path_sd;
   ...
   * Convert the character times to time variables;
   ...
   depart_hour = .;
   depart_hour = HOUR(q393_time);
   if q393_time = HMS(1,0,0) then depart_hour = .;
   if q394 = 1 then depart_hour = depart_hour;
   if q394 = 1 and HOUR(q393_time) = 12 then depart_hour = 0;
   if q394 = 2 and HOUR(q393_time) < 12 then depart_hour = depart_hour + 12;

   return_hour = .;
   return_hour = HOUR(q395_time);
   if q395_time = HMS(1,0,0) then return_hour = .;
   if q396 = 1 then return_hour = return_hour;
   if q396 = 1 and HOUR(q395_time) = 12 then return_hour = 0;
   if q396 = 2 and HOUR(q395_time) < 12 then return_hour = return_hour + 12;
...
data catiB; set catiA; keep id weight route daypart weekpart orig_purp dest_purp tour_purp autoSuff access_mode survey_mode egress_mode
...
data OnBoard.sam_cati_ready; set catiB;
run;
...
data infoA; set OnBoard.rawSamInfo;
...
data OnBoard.sam_info_ready; set infoC; keep id route direction field_language orig_purp_field dest_purp_field survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y last_alighting_x last_alighting_y;
...
data dowA; set OnBoard.rawSamDowWkday; keep survey_id day_of_the_week;
   rename survey_id = id;

data dowB; set OnBoard.rawSamDowWkend; keep survey_id day_of_the_week;
   rename survey_id = id;

proc append base = dowA data = dowB force;

data OnBoard.sam_dow_ready; set dowA;
   label day_of_week = "Day of week of field survey";
run;


* Merge the data sets;
data catiA; set OnBoard.sam_cati_ready;
data locA;  set OnBoard.sam_key_loc_taz;
data infoA; set OnBoard.sam_info_ready;
data dowA;  set OnBoard.sam_dow_ready;
...
*                   '12345678901234567890123456789012345678901234567890';
   field_start    = MDY(4,15,2013); 
   field_end      = MDY(5,11,2013);

data OnBoard.sam_ready; set comboC;
run;
