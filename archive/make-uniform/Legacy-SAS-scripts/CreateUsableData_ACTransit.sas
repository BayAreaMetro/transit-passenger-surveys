...
*          See BuildDatabases.sas for script to create SAS databases.  Specific to AC Transit.
*          Work in progress.
*
...
%let weekday_gis_geocode     = 'M:\Data\OnBoard\Data and Reports\AC Transit\Geocode\gis_geocode_weekday.csv';
%let weekend_gis_geocode     = 'M:\Data\OnBoard\Data and Reports\AC Transit\Geocode\gis_geocode_weekend.csv';
%let weekday_gis_geocode_maz = 'M:\Data\OnBoard\Data and Reports\AC Transit\Geocode\gis_geocode_weekday_maz.csv';
%let weekend_gis_geocode_maz = 'M:\Data\OnBoard\Data and Reports\AC Transit\Geocode\gis_geocode_weekend_maz.csv';
...
* - CATI;
data day; set OnBoard.rawAcTransitWeekdayCati;
   weekpart = 'WEEKDAY';
run;
 
data end; set OnBoard.rawAcTransitWeekendCati;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawAcTransitCati; set day;
run;

* - GIS;
data day; set OnBoard.rawAcTransitWeekdayGis;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawAcTransitWeekendGis;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawAcTransitGis; set day;
run;

* - Info;
data day; set OnBoard.rawAcTransitWeekdayInfo;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawAcTransitWeekendInfo;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawAcTransitInfo; set day;
run;
...
* Trip purpose;
data catiA; set OnBoard.RawAcTransitCati;
   orig_purp = "                         ";
   dest_purp = "                         ";
...
data catiA; set catiA;
   ...
   if q10 = 1 and q11 = 1 and q12 = 6  and q405>97 then orig_purp = "grade school";
   ...
   if q10 = 1 and q11 = 2 and q12 = 6  and q405>97 then dest_purp = "grade school";
...
data OnBoard.ac_cati_purp; set catiD;
...
data catiF; set OnBoard.ac_cati_purp; keep id q382 q390 q391 q392 orig_purp dest_purp tour_purp;
run;
...
* Weekday;
data day; infile "&weekday_gis_geocode." delimiter = ',' missover scanover dsd
...
* Weekend;
data end; infile "&weekend_gis_geocode." delimiter = ',' missover scanover dsd
            lrecl = 32767 firstobs = 2;
...
data day_maz; infile "&weekday_gis_geocode_maz." delimiter = ',' missover scanover dsd
            lrecl = 32767 firstobs = 2;
...
data end_maz; infile "&weekend_gis_geocode_maz." delimiter = ',' missover scanover dsd
            lrecl = 32767 firstobs = 2;
...
data gisA;  set OnBoard.RawAcTransitGis;
data catiA; set OnBoard.ac_cati_purp; keep id q10 q11;
data geoA;  set input;
...
data OnBoard.AC_key_loc_taz; set geoE; drop _TYPE_ _FREQ_;
run;

* Set the home and work locations if they are an origin or a destionation;
data catiA; set OnBoard.ac_cati_purp; keep id orig_purp dest_purp;
data gisA; set OnBoard.ac_key_loc_taz;
...
data OnBoard.AC_key_loc_taz; set gisC; drop orig_purp dest_purp;
run;
...
data catiA; set OnBoard.ac_cati_purp;
...
data OnBoard.AC_cati_purp_asuff; set catiB;
...
%MACRO TM_MODE(LABEL,MODE_Q,AC_SEL_Q,AC_Q,MUNI_Q,SAM_Q,VTA_Q,TRANSFER);
   ...
   * AC Transit;
   ...
   if &MODE_Q = 1 and &AC_SEL_Q = 2 and (&AC_Q < 57 or &AC_Q > 88) then &LABEL = 'local bus';
...
* Set the access mode;
data catiA; set OnBoard.AC_cati_purp_asuff;
...
data catiB; set catiA;
   ...
   survey_mode = put('missing',15.);
   survey_mode = 'local bus';
   if trim(route) = 'B'   then survey_mode = 'express bus';
   if trim(route) = 'C'   then survey_mode = 'express bus';
   if trim(route) = 'CB'  then survey_mode = 'express bus';
   if trim(route) = 'DA'  then survey_mode = 'express bus';
   if trim(route) = 'DB'  then survey_mode = 'express bus';
   if trim(route) = 'DB1' then survey_mode = 'express bus';
   if trim(route) = 'E'   then survey_mode = 'express bus';
   if trim(route) = 'F'   then survey_mode = 'express bus';
   if trim(route) = 'FS'  then survey_mode = 'express bus';
   if trim(route) = 'G'   then survey_mode = 'express bus';
   if trim(route) = 'H'   then survey_mode = 'express bus';
   if trim(route) = 'J'   then survey_mode = 'express bus';
   if trim(route) = 'L'   then survey_mode = 'express bus';
   if trim(route) = 'LA'  then survey_mode = 'express bus';
   if trim(route) = 'LC'  then survey_mode = 'express bus';
   if trim(route) = 'M'   then survey_mode = 'express bus';
   if trim(route) = 'NL'  then survey_mode = 'express bus';
   if trim(route) = 'NX'  then survey_mode = 'express bus';
   if trim(route) = 'NX1' then survey_mode = 'express bus';
   if trim(route) = 'NX2' then survey_mode = 'express bus';
   if trim(route) = 'NX3' then survey_mode = 'express bus';
   if trim(route) = 'NX4' then survey_mode = 'express bus';
   if trim(route) = 'NXC' then survey_mode = 'express bus';
   if trim(route) = 'O'   then survey_mode = 'express bus';
   if trim(route) = 'OX'  then survey_mode = 'express bus';
   if trim(route) = 'P'   then survey_mode = 'express bus';
   if trim(route) = 'S'   then survey_mode = 'express bus';
   if trim(route) = 'SB'  then survey_mode = 'express bus';
   if trim(route) = 'U'   then survey_mode = 'express bus';
   if trim(route) = 'V'   then survey_mode = 'express bus';
   if trim(route) = 'W'   then survey_mode = 'express bus';
   if trim(route) = 'Z'   then survey_mode = 'express bus';
...
data OnBoard.AC_cati_purp_asuff_path; set catiE;
...
data catiA; set OnBoard.AC_cati_purp_asuff_path; 
   ...
   hispanic = 'Missing                                                    ';
   if q406 = 1 then hispanic = 'HISPANIC/LATINO OR OF SPANISH ORIGIN';
   if q406 = 2 then hispanic = 'NOT HISPANIC/LATINO OR OF SPANISH ORIGIN'; 
   
   race = 'Missing            ';
   if q407 = 1 then race = 'WHITE';
   if q407 = 2 then race = 'BLACK';
   if q407 = 3 then race = 'ASIAN';
   if q407 = 4 then race = 'OTHER';
   if q407 = 5 then race = 'OTHER';
   if q407 = 6 then race = 'Missing            ';
   if q407 = 7 then race = 'OTHER';
   if q407 = 8 then race = 'OTHER';
   ...
   if q414 = 1 then interview_language = 'English';
   if q414 = 2 then interview_language = 'Spanish';
   if q414 = 3 then interview_language = 'Cantonese Chinese';
   if q414 = 4 then interview_language = 'Mandarin Chinese';
...
data OnBoard.AC_cati_purp_asuff_path_sd; set catiA;
...
data catiA; set OnBoard.AC_cati_purp_asuff_path_sd;

   depart_hour = .;
   depart_hour = HOUR(q393);
   if q393 = HMS(1,0,0) then depart_hour = .;
   if q394 = 1 then depart_hour = depart_hour;
   if q394 = 1 and HOUR(q393) = 12 then depart_hour = 0;
   ...
   if q396 = 1 and HOUR(q395) = 12 then return_hour = 0;
...
data catiB; set catiA; keep id weight orig_purp dest_purp tour_purp autoSuff access_mode survey_mode egress_mode
                            path_access path_egress path_line_haul path_label transfer_from transfer_to
							first_transit_mode last_transit_mode
							boardings work_status student_status
                            age fare_medium fare_category hispanic race language_at_home household_income sex 
                            interview_language depart_hour return_hour weekpart;
...
data OnBoard.AC_cati_ready; set catiB;
run;


* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~;
* Step 8: Extract relevant data from the INFO and GIS and DOW data sets and build combine data set;
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~;

data infoA; set OnBoard.RawAcTransitInfo;

   orig_purp_field = "                         ";
   dest_purp_field = "                         ";

   if origin_type_code = 1  then orig_purp_field = "work";
   if origin_type_code = 2  then orig_purp_field = "work-related";
   ...
   if dest_type_code = 2  then dest_purp_field = "work-related";
...
data OnBoard.ac_info_ready; set infoC; keep id route direction daypart field_language orig_purp_field dest_purp_field survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y last_alighting_x last_alighting_y;
...
data dowA; set OnBoard.rawAcTransitWeekdayDow; keep survey_id day_of_the_week;
   rename survey_id = id;

data dowB; set OnBoard.rawAcTransitWeekendDow; keep survey_id day_of_the_week;
   rename survey_id = id;

proc append base = dowA data = dowB force;

data OnBoard.ac_dow_ready; set dowA;
   label day_of_week = "Day of week of field survey";
run;


* Merge the data sets;
data catiA; set OnBoard.ac_cati_ready;
data infoA; set OnBoard.ac_info_ready;
data locA;  set OnBoard.ac_key_loc_taz;
data dowA;  set OnBoard.ac_dow_ready;
...
data comboC; merge comboB dowA; by id;

data comboD; set comboC;
   if weight = . then survey_type = 'brief';
run;

proc freq data = comboD; tables survey_type;
run;

* Add the operator, field start, and field end information;
data readyA; set comboD;
   operator    = 'AC Transit                                        ';
*                '12345678901234567890123456789012345678901234567890';
   field_start = MDY(9,13,2012); 
   field_end   = MDY(12,20,2012);

data OnBoard.ac_ready; set readyA;
run;
