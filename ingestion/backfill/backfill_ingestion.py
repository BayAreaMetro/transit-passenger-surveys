...
def ingest_survey_batches(
    ...
    """Ingest survey data to the Parquet data warehouse.
...
def extract_and_ingest_weights(
    ...
    """Extract survey weights and write to data warehouse.
    ...
    # Write weights to data warehouse (will fail on first error)
    ingest_survey_weights(weights_df, validate=True, archive=False)
    return len(weights_df)
