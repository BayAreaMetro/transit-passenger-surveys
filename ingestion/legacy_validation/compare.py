...
def load_legacy_data(operator: str, year: int) -> pl.DataFrame:
    """Load survey data from existing Parquet files.
    ...
    """
    logger.info("Loading data for %s %s from Parquet...", operator, year)

    responses = pl.read_parquet(DATA_ROOT / "survey_responses.parquet")
    legacy_df = responses.filter(
        (pl.col("canonical_operator") == operator) & (pl.col("survey_year") == year)
    )

    logger.info("Loaded %s records", f"{len(legacy_df):,}")
