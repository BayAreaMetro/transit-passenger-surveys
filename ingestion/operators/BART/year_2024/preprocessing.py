...
from transit_passenger_tools.codebook import EnglishProficiency, Gender, Race
from transit_passenger_tools.pipeline.geocoding import geocode_stops_from_names
...
def setup_logging(output_dir: str | None = None) -> None:
    ...
    if output_dir and (Path(output_dir).exists() or Path(output_dir).drive == ""):
        ...
        if output_dir:
            logger_temp.warning(
                "Output directory %s not accessible, using local directory: %s", output_dir, log_dir
            )
...
def process_demographics(  # noqa: PLR0912, PLR0915
    ...
    # Map "No Answer" → "Prefer not to answer"; null out any remaining non-enum values
    survey_df = survey_df.with_columns(
        pl.when(pl.col("gender") == "No Answer")
        .then(pl.lit(Gender.PREFER_NOT_TO_ANSWER.value))
        .when(pl.col("gender").is_in(_valid_genders))
        .then(pl.col("gender"))
        .otherwise(None)
        .alias("gender")
    )
    ...
    # Map SingleRaceCode to race_cat using Race enum values
    race_cat_mapping = {
        "American Indian, non-Hispanic": Race.OTHER.value,
        "Asian or Pac Islander, non-Hispanic": Race.ASIAN.value,
        "African American, non-Hispanic": Race.BLACK.value,
        "White or MENA, non-Hispanic": Race.WHITE.value,
        "Hispanic, any race": Race.OTHER.value,
        "Multi-racial, non-Hispanic": Race.OTHER.value,
        "No response": None,
        "Other, non-Hispanic": Race.OTHER.value,
    }
...
def process_trip_characteristics(  # noqa: PLR0915
    ...
    # Map BART place type codes to canonical Purpose values
    # DESTIN_PLACE_FINAL & ORIGIN_PLACE_TYPE_FINAL share the same code scheme:
    # 1 Home
    # 2 Workplace
    # 3 Work-related or business appointment
    # 4 School
    # 5 Shopping
    # 6 Restaurant or cafe
    # 7 Theater, concert, sports event or similar
    # 8 Other social or recreational (includes visiting friends or family)
    # 9 Personal errands (bank, medical appointment, DMV, etc.)
    # 10 Airplane trip
    # 11 Hotel (or other short-term residence)
    # 12 Picking up or dropping off others (dest only; orig field uses 99 = "Don't know")
    # 99 No Answer / Don't know -> None
    logger.info("Mapping ORIGIN_PLACE_TYPE_FINAL and DESTIN_PLACE_FINAL to orig_purp and dest_purp")

    place_purpose_mapping = {
        1: "Home",
        2: "Work",
        3: "Work-related",
        4: "School",
        ...
        11: "Hotel",
        12: "Escorting",
        # 99 = No Answer / Don't know -> handled by default=None below
    }
    ...
    bart_df = bart_df.with_columns(
        [
            ...
            # False for any other valid code, None for No Answer (99) or missing
            pl.when(
                pl.col("ORIGIN_PLACE_TYPE_FINAL").is_null()
                | pl.col("ORIGIN_PLACE_TYPE_FINAL").is_in([99])
            )
            .then(pl.lit(None).cast(pl.Boolean))
            .otherwise(pl.col("ORIGIN_PLACE_TYPE_FINAL") == 2)  # noqa: PLR2004
            .alias("at_work_prior_to_orig_purp"),
            pl.when(
                pl.col("ORIGIN_PLACE_TYPE_FINAL").is_null()
                | pl.col("ORIGIN_PLACE_TYPE_FINAL").is_in([99])
            )
            .then(pl.lit(None).cast(pl.Boolean))
            .otherwise(pl.col("ORIGIN_PLACE_TYPE_FINAL") == 4)  # noqa: PLR2004
            .alias("at_school_prior_to_orig_purp"),
            pl.when(
                pl.col("DESTIN_PLACE_FINAL").is_null() | pl.col("DESTIN_PLACE_FINAL").is_in([99])
            )
            .then(pl.lit(None).cast(pl.Boolean))
            .otherwise(pl.col("DESTIN_PLACE_FINAL") == 2)  # noqa: PLR2004
            .alias("at_work_after_dest_purp"),
            pl.when(
                pl.col("DESTIN_PLACE_FINAL").is_null() | pl.col("DESTIN_PLACE_FINAL").is_in([99])
            )
            .then(pl.lit(None).cast(pl.Boolean))
            .otherwise(pl.col("DESTIN_PLACE_FINAL") == 4)  # noqa: PLR2004
    ...
    bart_df = bart_df.with_columns(
        [
            ...
            .replace({"Unknown": "Don't know/refuse"})
    ...
    if fare_cat_lookup:
        bart_df = bart_df.with_columns(
            [
                pl.col("TYPE_OF_FARE")
                .replace_strict(fare_cat_lookup, default=None, return_dtype=pl.Utf8)
                .alias("fare_category")
            ]
        )
    else:
    ...
    if work_status_lookup:
        bart_df = bart_df.with_columns(
            [
                pl.col("work_status")
                .replace_strict(work_status_lookup, default=None, return_dtype=pl.Utf8)
                .alias("work_status")
            ]
        )
    ...
    if student_status_lookup:
        bart_df = bart_df.with_columns(
            [
                ...
                .replace_strict(student_status_lookup, default=None, return_dtype=pl.Utf8)
    ...
    if hh_income_lookup:
        bart_df = bart_df.with_columns(
            [
                ...
                .replace_strict(hh_income_lookup, default=None, return_dtype=pl.Utf8)
...
def preprocess(
    ...
    # Route field will be generated by pipeline from operator + stations
    # No need to add it here

    # Add direction field (BART doesn't have traditional direction data)
    logger.info("Adding direction field")
    survey_df = survey_df.with_columns([pl.lit(None).alias("direction")])
    ...
    survey_df = survey_df.with_columns(
        [
            ...
            # race field (combine race_cat or null)
            pl.col("race_cat").alias("race"),
            # language_at_home (use language_at_home_binary)
            pl.col("language_at_home_binary").alias("language_at_home"),
        ]
    )
    ...
    # Convert eng_proficient from numeric code to EnglishProficiency enum value
    _eng_proficient_map = {
        "1": EnglishProficiency.VERY_WELL.value,
        "2": EnglishProficiency.WELL.value,
        "3": EnglishProficiency.NOT_WELL.value,
        "4": EnglishProficiency.NOT_AT_ALL.value,
    }
    survey_df = survey_df.with_columns(
        pl.col("eng_proficient")
        .cast(pl.Utf8, strict=False)
        .replace_strict(_eng_proficient_map, default=None, return_dtype=pl.Utf8)
