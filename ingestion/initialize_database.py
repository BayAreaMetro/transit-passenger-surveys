...
from src.transit_passenger_tools import db
...
def create_folder_structure() -> None:
    ...
    for name in ["survey_responses", "survey_metadata", "survey_weights"]:
        (db.DATA_LAKE_ROOT / name).mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    if not db.DATA_LAKE_ROOT.parent.exists():
        logger.error("Network share not accessible: %s", db.DATA_LAKE_ROOT.parent)
        sys.exit(1)
    ...
    logger.info("\nData lake initialized: %s", db.DATA_LAKE_ROOT)
