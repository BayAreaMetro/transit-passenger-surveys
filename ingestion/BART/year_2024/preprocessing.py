...
def process_trip_characteristics(  # noqa: PLR0915
    ...
    # Map BART place type codes to canonical TripPurpose values
...
def preprocess(
    ...
    survey_df = survey_df.with_columns(
        [
            ...
            (pl.col("date_string").cast(pl.Utf8) + " " + pl.col("time_string").cast(pl.Utf8)).alias(
                "survey_time"
            ),
            ...
            pl.when(pl.col("race_cat").is_not_null())
            .then(pl.col("race_cat"))
            .otherwise(pl.lit("Missing"))
            .alias("race"),
            # language_at_home (use language_at_home_binary)
            pl.when(pl.col("language_at_home_binary").is_not_null())
            .then(pl.col("language_at_home_binary"))
            .otherwise(pl.lit("Missing"))
