...
#### Status
beta version with external inputs -- need to finish, check, refine
...
3. Can we put common elements into an R Package?
...
```{r incidence-matrix}
# TODO: add some checking, debugging, and formalize special case static names
...
names(unique_weights)[names(unique_weights) == "one"] <- all_routes_column_name 

# To create the rest of the incidence matrix, add a column of ones to the observed_targets dataframe
observed_targets <- observed_targets %>%
  mutate(one = 1)

# Pivot the incidence matrix off unique weights
...
for (i in 1:length(target_categories_vector)) {
  ...
  for (j in 1:length(variable_names)) {
    ...
    if(variable_names[j] == all_routes_string) {
...
# ~X seconds (~Y minutes) (was ~35 before)

sum_unique_weights <- as.data.frame(t(coef(optimx_results)))
names(sum_unique_weights)[1] <- "sum_weights"
```

# START HERE: if the optimization ran, begin here and work down
...
target_compare <- left_join(target_compare, target, by = c("route", "direction", "time_period"))
...
target <- observed_targets %>%
  filter(target_category_id == "C")

target_compare <- survey %>%
  group_by(route, direction, board_seg_4) %>%
  summarise(sum_base_weights = sum(weight), sum_optim_weights = sum(record_weight))

target_compare <- left_join(target_compare, target, by = c("route", "direction", "board_seg_4"))
...
target <- observed_targets %>%
  filter(target_category_id == "D")

target_compare <- survey %>%
  group_by(route, direction, alight_seg_4) %>%
  summarise(sum_base_weights = sum(weight), sum_optim_weights = sum(record_weight))

target_compare <- left_join(target_compare, target, by = c("route", "direction", "alight_seg_4"))
...
target <- observed_targets %>%
  filter(target_category_id == "E")

target_compare <- survey %>%
  group_by(route, direction, board_seg_3, alight_seg_3) %>%
  summarise(sum_base_weights = sum(weight), sum_optim_weights = sum(record_weight))

target_compare <- left_join(target_compare, target, by = c("route", "direction", "board_seg_3", "alight_seg_3"))
