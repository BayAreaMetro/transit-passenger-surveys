...
# In lieu of canonical database, experiment with just muni, then BART (clean up to do so)
working_df <- running_df %>%
  mutate(survey_operator = ifelse(survey_operator == "MUNI", "SF Muni", survey_operator)) %>%
  filter((survey_name == "BART" | survey_name == "SF Muni") & (survey_operator == "SF Muni" | survey_operator == "BART")) %>%
  mutate(survey_route = str_replace(survey_route, "Muni Route ", "")) %>%
  mutate(survey_route = str_replace(survey_route, "Muni  ", "")) %>%
  mutate(survey_route = str_replace(survey_route, " Light Rail", "")) %>%
  mutate(survey_route = str_replace(survey_route, "MUNI ", "")) %>%
  mutate(survey_route = str_replace(survey_route, " \\[ INBOUND \\]", "")) %>%
  mutate(survey_route = str_replace(survey_route, " \\[ OUTBOUND \\]", "")) %>%
  mutate(survey_route = str_replace(survey_route, "-", " ")) %>%
  group_by(survey_operator, survey_route, survey_name, survey_year, weekpart, survey_label, survey_detail_label) %>%
...
# TODO check why third_after goes away
boardings_df <- working_df %>%
  select(-survey_trips, -survey_label) %>%
  spread(key = survey_detail_label, value = survey_boardings, fill = 0.0) %>%
  mutate(third_after = 0)

trips_df <- working_df %>%
  filter(survey_label == "intercept") %>%
  mutate(survey_trips = ifelse(is.na(survey_trips), 0, survey_trips)) %>%
  select(operator, route, route_full, weekpart, survey_name, survey_year, survey_trips)

output_df <- left_join(boardings_df, 
                       trips_df, 
                       by = c("operator", "route", "route_full", 
                              "weekpart", "survey_name", "survey_year"))
