...
*          See BuildDatabases.sas for script to create SAS databases.  Specific to LAVTA.
*          Work in progress.
*
...
%let gis_geocode_weekday     = 'M:\Data\OnBoard\Data and Reports\LAVTA\Geocode\gis_geocode_weekday.csv';
%let gis_geocode_weekend     = 'M:\Data\OnBoard\Data and Reports\LAVTA\Geocode\gis_geocode_weekend.csv';
%let gis_geocode_maz_weekday = 'M:\Data\OnBoard\Data and Reports\LAVTA\Geocode\gis_geocode_weekday_maz.csv';
%let gis_geocode_maz_weekend = 'M:\Data\OnBoard\Data and Reports\LAVTA\Geocode\gis_geocode_weekend_maz.csv';
%let output_directory = M:\Data\OnBoard\Data and Reports\LAVTA\_working;
run;

* Add weekpart and combine weekend and weekday files;
* - CATI;
data day; set OnBoard.rawLavtaCatiWkday;
   weekpart = 'WEEKDAY';
   q408_number = input(q408,5.0);
   drop q408;
   rename q408_number = q408;
   if q408 = . then q408 = 1;
run;
 

data end; set OnBoard.rawLavtaCatiWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawLavtaCati; set day;
run;

* - GIS;
data day; set OnBoard.rawLavtaGisWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawLavtaGisWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawLavtaGis; set day;
run;

* - Info;
data day; set OnBoard.rawLavtaInfoWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawLavtaInfoWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawLavtaInfo; set day;
run;
...
data catiA; set OnBoard.rawLavtaCati;
   orig_purp = "                         ";
   dest_purp = "                         ";
...
data OnBoard.lavta_cati_purp; set catiD;
...
data gisA;  set OnBoard.rawLavtaGis;
data catiA; set OnBoard.lavta_cati_purp; keep id q10 q11;
data geoA;  set input;

proc sort data = gisA threads; by id q;
proc sort data = catiA threads; by id;
proc sort data = geoA threads; by id q;

data geoB; merge gisA geoA; by id q;
...
data OnBoard.lavta_key_loc_taz; set geoE; drop _TYPE_ _FREQ_;
run;


* Set the home and work locations if they are an origin or a destionation;
data catiA; set OnBoard.lavta_cati_purp; keep id orig_purp dest_purp;
data gisA; set OnBoard.lavta_key_loc_taz;
run;
...
data OnBoard.lavta_key_loc_taz; set gisC; drop orig_purp dest_purp;
run;
...
data catiA; set OnBoard.lavta_cati_purp;
...
data OnBoard.lavta_cati_purp_asuff; set catiB;
...
%MACRO TM_MODE(LABEL,MODE_Q,AC_SEL_Q,AC_Q,MUNI_Q,SAM_Q,VTA_Q,TRANSFER);
   ...
   * WHEELS (LAVTA);
   ...
   if &MODE_Q = 21 and &TRANSFER = 0 then transfer_from = 'WHEELS (LAVTA)';
   if &MODE_Q = 21 and &TRANSFER = 1 then transfer_to   = 'WHEELS (LAVTA)';
...
* Set the access mode;
data catiA; set OnBoard.lavta_cati_purp_asuff;
...
data OnBoard.lavta_cati_purp_asuff_path; set catiE;
...
data catiA; set OnBoard.lavta_cati_purp_asuff_path; 
   ...
   if q400 = 2   then fare_medium = 'pass (10 fare bust adult and student)';
   if q400 = 3   then fare_medium = 'pass (10 fare bust senior and disabled)';
   if q400 = 4   then fare_medium = 'pass (promotion)';
   if q400 = 5   then fare_medium = 'transfer (WHEELS)';
   if q400 = 6   then fare_medium = 'transfer (BART)';
   if q400 = 7   then fare_medium = 'transfer (ACE)';
   if q400 = 8   then fare_medium = 'transfer (County Connection)';
   if q400 = 9   then fare_medium = 'pass (hacienda park)';
   if q400 = 10  then fare_medium = 'pass (east bay value)';
   if q400 = 11  then fare_medium = 'pass (monthly senior)';
   if q400 = 12  then fare_medium = 'pass (monthly disabled)';
   if q400 = 13  then fare_medium = 'pass (waac disabled)';
   if q400 = 14  then fare_medium = 'pass (employee)';
   if q400 = 15  then fare_medium = 'other';

   fare_category = 'Missing                             ';
   if q401 = 1 then fare_category = 'adult';
   if q401 = 2 then fare_category = 'senior';
   if q401 = 3 then fare_category = 'disabled';
   if q401 = 4 then fare_category = 'other discount';
   if q401 = 5 then fare_category = 'free';
   ...
   * Match AC Transit: BLACK, WHITE, ASIAN, OTHER, MULTIRACIAL;
   ...
   if q410 = 4 then race = 'ASIAN';
   if q410 = 5 then race = 'OTHER';
   if q410 = 6 then race = 'OTHER';
   if q410 = 7 then race = 'OTHER';
   if q410 = 9 then race = 'OTHER';
   ...
   * ENGLISH ONLY, ...;
   ...
   if q412 = 10 then language_at_home = 'HINDI';
...
data OnBoard.lavta_cati_purp_asuff_path_sd; set catiA;
...
data catiA; set OnBoard.lavta_cati_purp_asuff_path_sd;
...
data OnBoard.lavta_cati_ready; set catiB;
run;
...
data infoA; set OnBoard.rawLavtaInfo;
...
proc summary data = gisA threads nway;
  class id;
  var survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y first_after_alighting_x first_after_alighting_y second_after_alighting_x second_after_alighting_y third_after_alighting_x third_after_alighting_y;
  output out = gisB
  max = survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y first_after_alighting_x first_after_alighting_y second_after_alighting_x second_after_alighting_y third_after_alighting_x third_after_alighting_y;

data gisC; set gisB; keep id survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y first_after_alighting_x first_after_alighting_y second_after_alighting_x second_after_alighting_y third_after_alighting_x third_after_alighting_y;
run;
...
data OnBoard.lavta_info_ready; set infoC; keep id route direction field_language orig_purp_field dest_purp_field survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y last_alighting_x last_alighting_y;
...
data dowA; set OnBoard.rawLavtaDowWkday; keep survey_id day_of_the_week;
   rename survey_id = id;

data dowB; set OnBoard.rawLavtaDowWkend; keep survey_id day_of_the_week;
   rename survey_id = id;

proc append base = dowA data = dowB force;

data OnBoard.lavta_dow_ready; set dowA;
   label day_of_week = "Day of week of field survey";
run;


* Merge the data sets;
data catiA; set OnBoard.lavta_cati_ready;
data locA;  set OnBoard.lavta_key_loc_taz;
data infoA; set OnBoard.lavta_info_ready;
data dowA;  set OnBoard.lavta_dow_ready;
...
*                   '12345678901234567890123456789012345678901234567890';
   field_start    = MDY(10,22,2013); 
   field_end      = MDY(10,30,2013);

data OnBoard.lavta_ready; set comboC;
run;
