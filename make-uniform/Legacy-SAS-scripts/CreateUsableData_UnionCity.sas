...
*          See BuildDatabases.sas for script to create SAS databases.  Specific to Union City.
*          Work in progress.
*
* Location: M:\Data\OnBoard\Data and Reports\
*
* Author(s): dto (2013 09 05)
*
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~;
...
%let gis_geocode     = 'M:\Data\OnBoard\Data and Reports\Union City\Geocode\gis_geocode.csv';
%let gis_geocode_maz = 'M:\Data\OnBoard\Data and Reports\Union City\Geocode\gis_geocode_maz.csv';
%let output_directory = M:\Data\OnBoard\Data and Reports\Union City\_working;
run;
...
* Trip purpose (q405 is year of birth);
data catiA; set OnBoard.rawUnionCati;
   orig_purp = "                         ";
   dest_purp = "                         ";
...
data OnBoard.union_cati_purp; set catiD;
...
* Read in CATI geo-codes (see \Geocoding Engine);
...
data gisA;  set OnBoard.rawUnionGis;
data catiA; set OnBoard.union_cati_purp; keep survey_id q10 q11;
   rename survey_id = id; 
data geoA;  set input;
...
data OnBoard.union_key_loc_taz; set geoE; drop _TYPE_ _FREQ_;
run;

* Set the home and work locations if they are an origin or a destination;
data catiA; set OnBoard.union_cati_purp; keep survey_id orig_purp dest_purp;
   rename survey_id = id;

data gisA; set OnBoard.union_key_loc_taz;
run;
...
data OnBoard.union_key_loc_taz; set gisC; drop orig_purp dest_purp;
run;
...
* q403 is number of workers, q404 is number of drivable vehicles;
data catiA; set OnBoard.union_cati_purp;
...
data OnBoard.union_cati_purp_asuff; set catiB;
...
* Set the access mode;
data catiA; set OnBoard.union_cati_purp_asuff;
...
proc freq data = catiE; tables trip_legs boardings boardings*trip_legs;
run;

data OnBoard.union_cati_purp_asuff_path; set catiE;
...
* check each of the questions;
data catiA; set OnBoard.union_cati_purp_asuff_path; 
   ...
   if q397 = 3   then fare_medium = 'pass (monthly)';
   if q397 = 4   then fare_medium = 'ticket (paratransit 10-ride)';
   if q397 = 5   then fare_medium = 'pass (clipper)';
   if q397 = 6   then fare_medium = 'pass (sticker on RTC card)';
   if q397 = 7   then fare_medium = 'ticket (paper plus cash)';
   if q397 = 8   then fare_medium = 'transfer (ac/dex)';
   if q397 = 9   then fare_medium = 'transfer (bart)';
   if q397 = 10  then fare_medium = 'transfer (bart plus pass)';
   if q397 = 11  then fare_medium = 'transfer (union city)';
   ...
   * Match AC Transit: BLACK, WHITE, ASIAN, OTHER, MULTIRACIAL;
   ...
   if q407 = 6 then race = 'OTHER';
   if q407 = 8 then race = 'OTHER';

   * Match AC Transit: ENGLISH ONLY, ...;
   language_at_home = 'Missing                 ';
   if q408 = 2 then language_at_home = 'ENGLISH ONLY';

   if q409 = 1  then language_at_home = 'SPANISH';
   if q409 = 2  then language_at_home = 'CHINESE-CANTONESE';
   if q409 = 3  then language_at_home = 'CHINESE-MANDARIN';
   if q409 = 4  then language_at_home = 'VIETNAMESE';
   if q409 = 5  then language_at_home = 'KOREAN';
   if q409 = 6  then language_at_home = 'TAGALOG';
   if q409 = 7  then language_at_home = 'RUSSIAN';
   if q409 = 8  then language_at_home = 'PORTUGUESE';
   if q409 = 9  then language_at_home = 'FRENCH';
   if q409 = 10 then language_at_home = 'FRENCH CREOLE';
   if q409 = 11 then language_at_home = 'POLISH';
   if q409 = 12 then language_at_home = 'OTHER'; 

   * English proficiency;
   eng_proficient = 'Missing                             ';
   if q410 = 1 then eng_proficient = 'VERY WELL';
   if q410 = 2 then eng_proficient = 'WELL';
   if q410 = 3 then eng_proficient = 'NOT WELL';
   if q410 = 4 then eng_proficient = 'NOT AT ALL';

   household_income = 'Missing                             ';
   if q411 = 1 then household_income = 'under $35,000';
   if q411 = 2 then household_income = '$35,000 or higher';
   if q411 = 3 then household_income = 'refused';

   if q412 = 1 then household_income = 'under $10,000';
   if q412 = 2 then household_income = '$10,000 to $25,000';
   if q412 = 3 then household_income = '$25,000 to $35,000';

   if q413 = 1 then household_income = '$35,000 to $50,000';
   if q413 = 2 then household_income = '$50,000 to $75,000';
   if q413 = 3 then household_income = '$75,000 to $100,000';
   if q413 = 4 then household_income = '$100,000 to $150,000';
   if q413 = 5 then household_income = '$150,000 or higher';

   sex = 'Missing                             '; 
   if q414 = 1 then sex = 'male';
   if q414 = 2 then sex = 'female';

   * CATI language (phone interview language);
   interview_language = 'Missing                             ';
   if q415 = 1 then interview_language = 'English';
   if q415 = 2 then interview_language = 'Spanish';
   if q415 = 3 then interview_language = 'Cantonese Chinese';
   if q415 = 4 then interview_language = 'Mandarin Chinese';
   if q415 = 5 then interview_language = 'Tagalog';
...
data OnBoard.union_cati_purp_asuff_path_sd; set catiA;
...
data catiA; set OnBoard.union_cati_purp_asuff_path_sd;
...
data catiB; set catiA; keep id weight route_surveyed daypart orig_purp dest_purp tour_purp autoSuff access_mode survey_mode egress_mode
...
data OnBoard.union_cati_ready; set catiB;
run;
...
data infoA; set OnBoard.rawUnionInfo;
...
data OnBoard.union_info_ready; set infoC; keep id route direction field_language orig_purp_field dest_purp_field survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y last_alighting_x last_alighting_y;
...
data dowA; set OnBoard.rawUnionDow; keep survey_id day_of_the_week;
   rename survey_id = id;

data OnBoard.union_dow_ready; set dowA;
   label day_of_week = "Day of week of field survey";
run;


* Merge the data sets;
data catiA; set OnBoard.union_cati_ready;
data locA;  set OnBoard.union_key_loc_taz;
data infoA; set OnBoard.union_info_ready;
data dowA;  set OnBoard.union_dow_ready;
...
*                   '12345678901234567890123456789012345678901234567890';
   field_start    = MDY(4,18,2013); 
   field_end      = MDY(4,26,2013);
   weekpart       = 'WEEKDAY';

data OnBoard.union_ready; set comboC;
run;
