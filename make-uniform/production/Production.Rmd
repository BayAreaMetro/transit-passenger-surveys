...
# START HERE ON GG FERRY (edit input CATI files, start on data dictionary)
...
```{r combine}
survey.combine <- rbind(survey.ACE.melt, survey.ACT.melt, survey.CCC.melt, survey.GGT.melt, survey.TRI.melt)
...
table(survey.cat$Survey_Variable)


temp <- survey.cat %.%
  filter(Survey_Variable == 'Destination_Type')

donkey <- left_join(temp, dictionary.cat, by = c("Operator", "Survey_Variable", "Survey_Response"))
