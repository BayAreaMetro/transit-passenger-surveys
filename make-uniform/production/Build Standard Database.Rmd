...
```{r}
rail_names_inputs <- read.csv(f_rail_names_inputs_path)
...
canonical_station_shp <- canonical_station_shp %>%
  ...
  # mutate(station_na = str_replace_all(station_na, "[.]", "")) %>%
  # mutate(station_na = str_replace_all(station_na, "Int'l", "International")) %>%
  # mutate(station_na = str_replace(route, "Oakland Airport", "Oakland International Airport")) %>%
  # mutate(station_na = str_replace_all(station_na, "/", " "))
...
remove(ac_transit_df, bart_df, caltrain_df, muni_df, napa_vine_df, vta_df)
...
survey_non <- survey_combine %>% 
  ...
  left_join(canonical_routes_crosswalk %>% select(-technology), by = c("operator" = "survey", "survey_year" = "survey_year", "survey_response" = "survey_name")) %>%
...
survey_flat <- bind_rows(survey_cat, survey_non) %>%
  ...
  mutate(route = ifelse(operator == "BART", paste("BART", onoff_enter_station, onoff_exit_station, sep = "___"), route)) %>%
  mutate(route = ifelse(operator == "Caltrain", paste("CALTRAIN", onoff_enter_station, onoff_exit_station, sep = "___"), route)) %>%
  mutate(route = str_replace(route, "Intl", "International"))
...
survey_flat <- survey_flat %>% 
  ...
  mutate(canonical_operator = ifelse(operator == "BART", "BART", canonical_operator),
         canonical_operator = ifelse(operator == "Caltrain", "CALTRAIN", canonical_operator)) %>%
  mutate(technology = ifelse(canonical_operator == "BART", "heavy rail", technology),
         technology = ifelse(canonical_operator == "CALTRAIN", "commuter rail", technology)) 
...
survey_standard <- survey_standard %>%
  mutate(first_operator_before = str_extract(first_route_before_survey_board, "^[A-z ]+?(?=_)"),
         second_operator_before = str_extract(second_route_before_survey_board, "^[A-z ]+?(?=_)"),
         third_operator_before = str_extract(third_route_before_survey_board, "^[A-z ]+?(?=_)"),
         first_operator_after = str_extract(first_route_after_survey_alight, "^[A-z ]+?(?=_)"),
         second_operator_after = str_extract(second_route_after_survey_alight, "^[A-z ]+?(?=_)"),
         third_operator_after = str_extract(third_route_after_survey_alight, "^[A-z ]+?(?=_)"))


# Set the technology for each of the six legs
tech_crosswalk <- canonical_routes_crosswalk %>% 
  select(-survey_name) %>%
  rename(temp_tech = technology)

survey_standard <- survey_standard %>%
  left_join(tech_crosswalk, by =c("operator" = "survey", "survey_year", 
                                  "first_operator_before" = "canonical_operator", 
                                  "first_route_before_survey_board" = "canonical_name")) %>%
  mutate(temp_tech = ifelse(canonical_operator == "BART", "heavy rail", temp_tech)) %>%
  mutate(temp_tech = ifelse(canonical_operator == "Caltrain", "commuter rail", temp_tech)) %>%
  rename(first_before_technology = temp_tech) %>%
  
  left_join(tech_crosswalk, by =c("operator" = "survey", "survey_year", 
                                  "second_operator_before" = "canonical_operator", 
  ...
  left_join(tech_crosswalk, by =c("operator" = "survey", "survey_year", 
                                  "third_operator_before" = "canonical_operator", 
  ...
  left_join(tech_crosswalk, by =c("operator" = "survey", "survey_year", 
                                  "first_operator_after" = "canonical_operator", 
  ...
  left_join(tech_crosswalk, by =c("operator" = "survey", "survey_year", 
                                  "second_operator_after" = "canonical_operator", 
  ...
  left_join(tech_crosswalk, by =c("operator" = "survey", "survey_year", 
                                  "third_operator_after" = "canonical_operator",
