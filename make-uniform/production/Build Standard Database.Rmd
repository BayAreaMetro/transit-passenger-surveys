...
taz_shp <- st_read(f_taz_shp_path) %>%
  ...
  bind_cols(match = 1:nrow(taz_shp))

maz_shp <- st_read(f_maz_shp_path) %>%
  select(maz = MAZ_ORIGIN) %>%
  bind_cols(match = 1:nrow(maz_shp))

survey_coords_spatial <- survey_coords_spatial %>%
  mutate(taz = st_nearest_feature(survey_coords_spatial, taz_shp),
         maz = st_nearest_feature(survey_coords_spatial, maz_shp)) %>%
  left_join(taz_shp, by = 
