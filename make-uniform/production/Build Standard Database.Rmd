...
board_alight_tap <- survey_board_spatial %>% select(-distance) %>%
  left_join(survey_alight_spatial %>% select(-distance), by = c("unique_ID"))
...
```{r geocode_other}
### Figure out projections
#Decimal degrees are 4236
# shape files are 2230?
...
temp <- st_distance(bad_survey_coords_spatial, taz_shp)

survey_coords_spatial <- survey_coords_spatial %>%
  mutate(taz_index = st_nearest_feature(survey_coords_spatial, taz_shp))
survey_coords_spatial <- survey_coords_spatial %>%
  mutate(maz_index = st_nearest_feature(survey_coords_spatial, maz_shp))

taz_distance <- survey_coords_spatial %>%
  select(taz_index)
st_geometry(taz_distance) <- NULL
taz_distance <- taz_shp %>%
  right_join(taz_distance, by = c("match" = "taz_index"))
taz_distance <- taz_distance %>%
  mutate(dist = st_distance(taz_shp, taz_distance, by_element = TRUE))
st_geometry(taz_distance) <- NULL
temp <- survey_coords_spatial %>%
  mutate(taz_dist = taz_distance$dist)
