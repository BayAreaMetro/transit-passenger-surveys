...
f_get_rail_names_inputs_path <- "get_rail_names_inputs.csv"
...
```{r}
get_rail_names_inputs <- read.csv(f_get_rail_names_inputs_path)
...
read_operator <- function(name, year, default_tech, file_path, variable_dictionary) {
  ...
  if (name %in% get_rail_names_inputs$survey_name) {
    inputs <- get_rail_names_inputs %>% 
      filter(survey_name_df == name)
    
    for (i in 1:nrow(get_rail_names_inputs %>% filter(survey_name_df == name))) {
...
## Recode Rail routes as Station to Station Pairs
```{r station-to-station-recode}
