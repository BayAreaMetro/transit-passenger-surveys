...
"user_lookup.csv" in this folder.
...
f_board_to_be_geocoded_path = paste0(dir_path, "Data and Reports/_geocoding Standardized/boarding_places_to_be_geocoded.csv")
f_alight_to_be_geocoded_path = paste0(dir_path, "Data and Reports/_geocoding Standardized/alighting_places_to_be_geocoded.csv")

f_spatial_geocoded_path = paste0(dir_path, "Data and Reports/_geocoding Standardized/spatial_places_geocoded.csv")
f_transit_geocoded_path = paste0(dir_path, "Data and Reports/_geocoding Standardized/boarding_alighting_places_geocoded.csv")

f_caltrain_survey_path = paste0(dir_path, "Data and Reports/Caltrain/As CSV/Caltrain_Final_Submitted_1_5_2015_TYPE_WEIGHT_DATE NO POUND OR SINGLE QUOTE.csv")

f_napa_survey_path = paste0(dir_path, "Data and Reports/Napa Vine/As CSV/Napa Vine Transit OD Survey Data_Dec10_Submitted_toAOK_with_transforms NO POUND OR SINGLE QUOTE.csv")

f_bart_survey_path = paste0(dir_path, "Data and Reports/BART/As CSV/BART_Final_Database_Mar18_SUBMITTED_with_station_xy_with_first_board_last_alight NO POUND OR SINGLE QUOTE.csv")

f_muni_survey_path = paste0(dir_path, "Data and Reports/Muni/As CSV/MUNI_DRAFTFINAL_20171114 NO POUND OR SINGLE QUOTE.csv")

f_vta_survey_path = paste0(dir_path, "Data and Reports/VTA/As CSV/VTA_DRAFTFINAL_20171114 NO POUND OR SINGLE QUOTE.csv")

f_output_rdata_path = paste0(dir_path, "Data and Reports/_data Standardized/survey_standard.RData")
f_output_csv_path = paste0(dir_path, "Data and Reports/_data Standardized/survey_standard.csv")
f_ancillary_output_rdata_path = paste0(dir_path, "Data and Reports/_data Standardized/ancillary_variables.RData")
f_ancillary_output_csv_path = paste0(dir_path, "Data and Reports/_data Standardized/ancillary_variables.csv")
f_output_decom_rdata_path = paste0(dir_path, "Data and Reports/_data Standardized/decomposition/survey_decomposition.RData")
f_output_decom_csv_path = paste0(dir_path, "Data and Reports/_data Standardized/decomposition/survey_decomposition.csv")
```
...
# Cast Factor ID to inte
survey.CAL.melt <- mutate(survey.CAL.melt, ID = as.numeric(ID))
...
### Muni
```{r operator-muni}
survey.MUN.key  <- 'SF Muni'
survey.MUN.year <- 2017
...
for (var in variables.MUN) {
  if (var %in% colnames(survey.MUN)==FALSE) {
    print(paste(var, var %in% colnames(survey.MUN)))
  }
}
...
```{r operator-VTA}
survey.VTA.key  <- 'VTA'
survey.VTA.year <- 2017

# Set survey tech by route if necessary in chunk `combine`
survey.VTA.tech <- 'local bus'

# Prepare operator-specific dictionary
dictionary.VTA <- dictionary.all %>%
  filter(operator == survey.VTA.key)

variables.VTA <- as.character(factor(unique(unlist(dictionary.VTA$Survey_Variable, use.names = TRUE))))

# Read data and select variables of interest
survey.VTA <- read.csv(F_VTA_SURVEY, header = TRUE)
for (var in variables.VTA) {
  if (var %in% colnames(survey.VTA)==FALSE) {
    print(paste(var, var %in% colnames(survey.VTA)))
  }
}
survey.VTA.select <- survey.VTA[ ,variables.VTA]

# Reshape the data
survey.VTA.melt <- melt(survey.VTA.select, id = 'id')
survey.VTA.melt <- select(survey.VTA.melt, id, Survey_Variable = variable, Survey_Response = value)
survey.VTA.melt <- mutate(survey.VTA.melt, operator = survey.VTA.key)
survey.VTA.melt <- mutate(survey.VTA.melt, survey_year = survey.VTA.year)
survey.VTA.melt <- mutate(survey.VTA.melt, survey_tech = survey.VTA.tech)
colnames(survey.VTA.melt)[1] <- "ID"

# Clean up
remove(survey.VTA.key, survey.VTA.year, survey.VTA.tech, dictionary.VTA, variables.VTA, survey.VTA, survey.VTA.select)
```
...
```{r combine}
survey.combine <- rbind(survey.BAR.melt, survey.CAL.melt, survey.MUN.melt, survey.VTA.melt, survey.NAP.melt)
...
ref_count       <- survey.cat %>% group_by(ID,operator,survey_year,survey_tech,Generic_Variable) %>% summarise( count=n() )
mult_ref_count  <- ref_count[which(ref_count$count>1),]
...
# NOTE: if below cast warns about missing aggregation function, likely because (a) duplicate unique ID or (b) more than three variables.
...
survey.decomposition <- survey.standard %>%
...
save(survey.decomposition, file = F_OUTPUT_DECOM_RDATA)
write.csv(survey.decomposition, file = F_OUTPUT_DECOM_CSV,  row.names = FALSE)
...
survey.standard <- survey.standard[,c(which(colnames(survey.standard)!="survey_time"),
                                      which(colnames(survey.standard)=="survey_time"))]

save(survey.standard, file = F_OUTPUT_RDATA)
save(ancillary_df, file = F_ANCILLARY_OUTPUT_RDATA)

write.csv(survey.standard, file = F_OUTPUT_CSV,  row.names = FALSE)
write.csv(ancillary_df,    file = F_ANCILLARY_OUTPUT_CSV,  row.names = FALSE)
