...
#### _TODO_
3.  Build full generalization with latest ETC
4.  Make geo-coding file location parameters (directory + file name) at the top
8.  Add data types before exporting to RData
9.  How to handle operators with mixed modes
10.  Separate script to munge legacy with current data and write out Tableau?
11.  One script to geo-code locations?
12.  Standard variables to add when real data comes in: survey language, expansion weight, survey type (tablet or paper)
...
table(survey.standard$fare_type)
...
write.csv(write.places, file = "M:/Data/OnBoard/Data and Reports/Geocoding Engine/spatial_places_to_be_geocoded.csv",  row.names = FALSE, quote = F)
write.csv(write.board,  file = "M:/Data/OnBoard/Data and Reports/Geocoding Engine/boarding_places_to_be_geocoded.csv", row.names = FALSE, quote = F)
write.csv(write.alight, file = "M:/Data/OnBoard/Data and Reports/Geocoding Engine/alighting_places_to_be_geocoded.csv", row.names = FALSE, quote = F)

remove(temp_lat, temp_lon, temp_lat.melt, temp_lon.melt)

```

```{r geocode-addit}
# START HERE read in, cast, join
geocoded_places <- read.csv('M:/Data/OnBoard/Data and Reports/Geocoding Engine/spatial_places_geocoded.csv', header = TRUE)
```
