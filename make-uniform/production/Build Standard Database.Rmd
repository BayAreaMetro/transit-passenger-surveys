...
#### _TODO_
1.  How to handle operators with mixed modes
2.  Separate script to (a) read in legacy data (b) munge legacy with current data --> write out Tableau and (c) write out public data set
3.  Standard variables to add when real data comes in: survey language, expansion weight, survey type (tablet or paper)
...
#### Remote file names
```{r file-names}
F_SPATIAL_TO_BE_GEOCODED = "M:/Data/OnBoard/Data and Reports/Geocoding Engine/spatial_places_to_be_geocoded.csv"
F_BOARD_TO_BE_GEOCODED   = "M:/Data/OnBoard/Data and Reports/Geocoding Engine/boarding_places_to_be_geocoded.csv"
F_ALIGHT_TO_BE_GEOCODED  = "M:/Data/OnBoard/Data and Reports/Geocoding Engine/alighting_places_to_be_geocoded.csv"

F_SPATIAL_GEOCODED = "M:/Data/OnBoard/Data and Reports/Geocoding Engine/spatial_places_geocoded.csv"
F_TRANSIT_GEOCODED = "M:/Data/OnBoard/Data and Reports/Geocoding Engine/boarding_alighting_places_geocoded.csv"

F_CALTRAIN_SURVEY = "M:/Data/OnBoard/Data and Reports/Caltrain/As CSV/Caltrain_PilotTest_MainSurvey_Database_SubmittedOct16 NO POUND OR SINGLE QUOTE.csv"

F_OUTPUT_RDATA = "M:/Data/OnBoard/Data and Reports/Standardized Data/_working/survey_standard.RData"
...
survey.CAL.melt <- mutate(survey.CAL.melt, survey_type = survey.CAL.type)

# Clean up
remove(survey.CAL.key, survey.CAL.year, survey.CAL.tech, survey.CAL.type, dictionary.CAL, variables.CAL, survey.CAL, survey.CAL.select)
```
...
# Add in survey technology, type
survey.adds <- survey.combine %>%
  group_by(ID, operator, survey_year, survey_tech, survey_type) %>%
  summarise(Count = n()) %>%
  select(-Count)
...
survey.standard <- survey.standard %>%
  mutate(date = as.Date(str_trim(str_sub(date_string, 1, 10)), format = "%m/%d/%Y")) %>%
  mutate(field_start = min(date)) %>%
  mutate(field_end   = max(date)) %>%
  # Day part (string manip to get time)
  mutate(time_start = as.numeric(str_trim(str_sub(date_string, -5, -4)))) %>%
  ...
  # Weekpart
  mutate(weekpart = ifelse(day_of_the_week == 'SATURDAY' | day_of_the_week == 'SUNDAY', 'WEEKEND', 'WEEKDAY'))
  

table(survey.standard$date)
...
table(survey.standard$weekpart)
...
table(survey.standard$home_maz)
table(survey.standard$home_taz)
table(survey.standard$first_board_tap)
...
## Write RData to Disk
```{r disk-write}
# Add missing variables (TODO: add when real data becomes available)
survey.standard <- survey.standard %>%
  mutate(weight = 0.0) %>%
  mutate(trip_weight = ifelse(boardings > 0, weight / boardings, weight)) %>%
  mutate(interview_language = 'Missing') %>%
  mutate(field_language = 'Missing')
