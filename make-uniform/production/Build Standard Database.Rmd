...
for (item in unique(taps_spatial$mode)) {
  ...
  st_geometry(temp_distance) <- NULL
  temp_board <- temp_board %>%
    ### Fix left join because it is set up to have a match for each row and the join criterion isn't exclusive.
    left_join(temp_distance %>% select(match, dist), by = c("temp_board_tap" = "match")) %>%
    mutate(distance = ifelse(!is.na(dist), dist, distance)) %>%
    select(-dist)
  ...
  st_geometry(temp_distance) <- NULL
  temp_alight <- temp_alight %>%
    left_join(temp_distance %>% select(match, dist), by = c("temp_alight_tap" = "match")) %>%
    mutate(distance = ifelse(!is.na(dist), dist, distance)) %>%
    select(-dist)
  ...
  survey_board_spatial <- survey_board_spatial %>%
    left_join(as.data.frame(temp_board) %>% select(unique_ID, temp_board_tap, temp_dist = distance), by = "unique_ID") %>%
  ...
  survey_alight_spatial <- survey_alight_spatial %>%
    left_join(as.data.frame(temp_alight) %>% select(unique_ID, temp_alight_tap, temp_dist = distance), by = "unique_ID") %>%
...
remove(alight_coords, board_coords, 
       survey_board_spatial, survey_alight_spatial, taps_coords, taps_spatial)
