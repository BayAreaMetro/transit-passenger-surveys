...
#### _TODO_
1.  Make sure BART has all the Napa and Caltrain variables before casting
2.  Check to make sure BART IDs are unique
3.  Finish BART debugging
...
F_BART_SURVEY = "M:/Data/OnBoard/Data and Reports/BART/As CSV/BART_Final_Database_Mar10_SUBMITTED NO POUND OR SINGLE QUOTE.csv"
...
# Set survey tech by route if necessary in standard variables Step 8
...
# debug start
# for (var in variables.BAR) {
#   
#   print(var)
#   donkey <- survey.BAR[ ,var]
#   
# }
# debug end
...
```{r combine}
survey.combine <- rbind(survey.BAR.melt, survey.CAL.melt, survey.NAP.melt)
...
remove(survey.BAR, melt, survey.CAL.melt, survey.NAP.melt)
...
Set_Operator_Name <- function(input_vector){
  ...
  output_df <- input_df %>%
    ...
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("AirBART")) > 0L, "AC TRANSIT", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("AirTrain")) > 0L, "BART", output_field)) %>%
    ...
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("Sam Trans")) > 0L, "SAMTRANS",  output_field)) %>%
    ...
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("DASH Downtown Area Shuttle")) > 0L, "VTA", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("Tiburon FERRY Tiburon To Angel Island State Park")) > 0L, "BLUE GOLD FERRY", output_field)) %>%
    ...
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("American Canyon Safeway")) > 0L,  "PRIVATE SHUTTLE", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("Shuttles Broadway - Millbrae")) > 0L,  "PRIVATE SHUTTLE", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("Calistoga Handy Van")) > 0L,  "OTHER", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("Lake Transit")) > 0L,  "OTHER", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("Saint Helena Shuttle")) > 0L,  "OTHER", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("Burlingame Trolley Shuttle")) > 0L,  "OTHER", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("SCMTD Highway 17")) > 0L,  "OTHER", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("Santa Cruz Metro")) > 0L,  "OTHER", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("Menlo Park Shuttle Midday Shuttle")) > 0L,  "OTHER", output_field)) %>%
    mutate(output_field = ifelse(!is.na(input_field) & str_count(input_field, ignore.case("Palo Alto E Embarcadero Shuttle")) > 0L,  "OTHER", output_field)) 
...
# Be-spoke survey technology updates, as needed
survey.standard <- survey.standard %>%
...
# Deal with time
survey.standard <- survey.standard %>%
  mutate(am_pm = str_trim(str_sub(time_string, 6, 8))) %>%
...
write.board <- survey.standard %>%
  ...
  mutate(first_board_alight = ifelse(number_transfers_orig_board == 0, survey_tech,      first_board_tech)) %>%
