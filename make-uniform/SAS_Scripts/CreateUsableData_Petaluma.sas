...
*          See BuildDatabases.sas for script to create SAS databases.  Specific to Petaluma
*          transit.
*          Work in progress.
*
* Location: M:\Data\OnBoard\Data and Reports\
*
* Author(s): dto (2013 07 XX)
*
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~;
...
%let gis_geocode     = 'M:\Data\OnBoard\Data and Reports\Petaluma\Geocode\gis_geocode.csv';
%let gis_geocode_maz = 'M:\Data\OnBoard\Data and Reports\Petaluma\Geocode\gis_geocode_maz.csv';
%let output_directory = M:\Data\OnBoard\Data and Reports\Petaluma\_working;
run;
...
data catiA; set OnBoard.rawPetalumaCati;
   orig_purp = "                         ";
   dest_purp = "                         ";
...
data OnBoard.pet_cati_purp; set catiD;
...
data gisA;  set OnBoard.rawPetalumaGis;
   rename surveyid = id;
   rename idcode   = qcode;
data catiA; set OnBoard.pet_cati_purp; keep id q10 q11;
data geoA;  set input;
...
data OnBoard.pet_key_loc_taz; set geoE; drop _TYPE_ _FREQ_;
run;


* Set the home and work locations if they are an origin or a destionation;
data catiA; set OnBoard.pet_cati_purp; keep id orig_purp dest_purp;
data gisA; set OnBoard.pet_key_loc_taz;
...
data OnBoard.pet_key_loc_taz; set gisC; drop orig_purp dest_purp;
run;
...
data catiA; set OnBoard.pet_cati_purp;
...
data OnBoard.pet_cati_purp_asuff; set catiB;
...
* Set the access mode;
data catiA; set OnBoard.pet_cati_purp_asuff;
...
data OnBoard.pet_cati_purp_asuff_path; set catiE;
...
data catiA; set OnBoard.pet_cati_purp_asuff_path; 
   ...
   fare_medium = 'Missing                             ';
   if q218 = 1  then fare_medium = 'transfer (bart)';
   if q218 = 2  then fare_medium = 'transfer (bus)';
   if q218 = 3  then fare_medium = 'cash (bills and coins)';
   if q218 = 4  then fare_medium = 'pass (31-day)';
   if q218 = 5  then fare_medium = 'pass (10-ride)';
   if q218 = 6 then fare_medium = 'other';
...
data OnBoard.pet_cati_purp_asuff_path_sd; set catiA;
...
data catiA; set OnBoard.pet_cati_purp_asuff_path_sd;
...
data OnBoard.pet_cati_ready; set catiB;
run;
...
data infoA; set OnBoard.rawPetalumaInfo;
...
data OnBoard.pet_info_ready; set infoC; keep id route direction field_language orig_purp_field dest_purp_field survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y last_alighting_x last_alighting_y;
...
data dowA; set OnBoard.rawPetalumaDow; keep survey_id day_of_the_week;
   rename survey_id = id;

data OnBoard.pet_dow_ready; set dowA;
...
* Merge the data sets;
data catiA; set OnBoard.pet_cati_ready;
data locA;  set OnBoard.pet_key_loc_taz;
data infoA; set OnBoard.pet_info_ready;
data dowA;  set OnBoard. pet_dow_ready;
...
*                   '12345678901234567890123456789012345678901234567890';
   field_start    = MDY(4,23,2012); 
   field_end      = MDY(4,23,2012);

data OnBoard.pet_ready; set comboC;
run;
