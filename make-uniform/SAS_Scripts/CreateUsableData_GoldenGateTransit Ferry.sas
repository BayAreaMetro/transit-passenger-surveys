...
*          Gate Transit (ferry service). Work in progress.
*
...
%let gis_geocode_weekday     = 'M:\Data\OnBoard\Data and Reports\Golden Gate Transit\Geocode\gis_geocode_weekday_ferry.csv';
%let gis_geocode_weekend     = 'M:\Data\OnBoard\Data and Reports\Golden Gate Transit\Geocode\gis_geocode_weekend_ferry.csv';
%let gis_geocode_maz_weekday = 'M:\Data\OnBoard\Data and Reports\Golden Gate Transit\Geocode\gis_geocode_weekday_ferry_maz.csv';
%let gis_geocode_maz_weekend = 'M:\Data\OnBoard\Data and Reports\Golden Gate Transit\Geocode\gis_geocode_weekend_ferry_maz.csv';
...
* - CATI;
data day; set OnBoard.rawGgtfCatiWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawGgtfCatiWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawGgtfCati; set day;
run;

* - GIS;
data day; set OnBoard.rawGgtfGisWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawGgtfGisWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawGgtfGis; set day;
run;

* - Info;
data day; set OnBoard.rawGgtfInfoWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawGgtfInfoWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawGgtfInfo; set day;
run;
...
data catiA; set OnBoard.rawGgtfCati;
   orig_purp = "                         ";
   dest_purp = "                         ";
...
data OnBoard.ggtf_cati_purp; set catiD;
...
data gisA;  set OnBoard.rawGgtfGis;
data catiA; set OnBoard.ggtf_cati_purp; keep id q10 q11;
data geoA;  set input;
...
data OnBoard.ggtf_key_loc_taz; set geoE; drop _TYPE_ _FREQ_;
run;


* Set the home and work locations if they are an origin or a destionation;
data catiA; set OnBoard.ggtf_cati_purp; keep id orig_purp dest_purp;
data gisA; set OnBoard.ggtf_key_loc_taz;
run;
...
data OnBoard.ggtf_key_loc_taz; set gisC; drop orig_purp dest_purp;
run;
...
data catiA; set OnBoard.ggtf_cati_purp;
...
data OnBoard.ggtf_cati_purp_asuff; set catiB;
...
* Set the access mode;
data catiA; set OnBoard.ggtf_cati_purp_asuff;
...
data OnBoard.ggtf_cati_purp_asuff_path; set catiE;
...
data catiA; set OnBoard.ggtf_cati_purp_asuff_path; 
...
data OnBoard.ggtf_cati_purp_asuff_path_sd; set catiA;
...
data catiA; set OnBoard.ggtf_cati_purp_asuff_path_sd;
...
data OnBoard.ggtf_cati_ready; set catiB;
run;
...
data infoA; set OnBoard.rawGgtfInfo;
...
data OnBoard.ggtf_info_ready; set infoC; keep id route direction field_language orig_purp_field dest_purp_field survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y last_alighting_x last_alighting_y;
...
data dowA; set OnBoard.rawGgtfDowWkday; keep survey_id day_of_the_week;
   rename survey_id = id;

data dowB; set OnBoard.rawGgtfDowWkend; keep survey_id day_of_the_week;
   rename survey_id = id;

proc append base = dowA data = dowB force;

data OnBoard.ggtf_dow_ready; set dowA;
...
* Merge the data sets;
data catiA; set OnBoard.ggtf_cati_ready;
data locA;  set OnBoard.ggtf_key_loc_taz;
data infoA; set OnBoard.ggtf_info_ready;
data dowA;  set OnBoard.ggtf_dow_ready;
...
data comboC; set comboB;
   operator       = 'Golden Gate Transit (ferry)                       ';
*                   '12345678901234567890123456789012345678901234567890';
   field_start    = MDY(09,18,2013); 
   field_end      = MDY(10,02,2013);

data OnBoard.ggtf_ready; set comboC;
run;
