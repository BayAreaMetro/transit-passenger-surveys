...
*          See BuildDatabases.sas for script to create SAS databases.  Specific to WETA. 
*          Work in progress.
*
...
%let gis_geocode_weekday     = 'M:\Data\OnBoard\Data and Reports\WETA\Geocode\gis_geocode_weekday.csv';
%let gis_geocode_weekend     = 'M:\Data\OnBoard\Data and Reports\WETA\Geocode\gis_geocode_weekend.csv';
%let gis_geocode_maz_weekday = 'M:\Data\OnBoard\Data and Reports\WETA\Geocode\gis_geocode_weekday_maz.csv';
%let gis_geocode_maz_weekend = 'M:\Data\OnBoard\Data and Reports\WETA\Geocode\gis_geocode_weekend_maz.csv';
%let output_directory = M:\Data\OnBoard\Data and Reports\WETA\_working;
run;

* Add weekpart and combine weekend and weekday files;
* - CATI;
data day; set OnBoard.rawWetaCatiWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawWetaCatiWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawWetaCati; set day;
run;

* - GIS;
data day; set OnBoard.rawWetaGisWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawWetaGisWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawWetaGis; set day;
run;

* - Info;
data day; set OnBoard.rawWetaInfoWkday;
   weekpart = 'WEEKDAY';

data end; set OnBoard.rawWetaInfoWkend;
   weekpart = 'WEEKEND';

proc append base = day data = end force;

data OnBoard.rawWetaInfo; set day;
run;
...
data catiA; set OnBoard.rawWetaCati;
   orig_purp = "                         ";
   dest_purp = "                         ";
...
data catiC; set catiB; 
  ...
  * If no back to work, but work is a part of the trip, then work;
...
data catiD; set catiC;
   ...
   * If you're still left, assume working before or after trip (which does not have a home or work end) puts you on a work tour;
   if tour_purp = "                         " and q390 < 3 then tour_purp = "work";
...
data OnBoard.weta_cati_purp; set catiD;
...
data gisA;  set OnBoard.rawWetaGis;
data catiA; set OnBoard.weta_cati_purp; keep id q10 q11;
data geoA;  set input;
...
data OnBoard.weta_key_loc_taz; set geoE; drop _TYPE_ _FREQ_;
run;


* Set the home and work locations if they are an origin or a destionation;
data catiA; set OnBoard.weta_cati_purp; keep id orig_purp dest_purp;
data gisA; set OnBoard.weta_key_loc_taz;
run;
...
data OnBoard.weta_key_loc_taz; set gisC; drop orig_purp dest_purp;
run;
...
data catiA; set OnBoard.weta_cati_purp;
...
data OnBoard.weta_cati_purp_asuff; set catiB;
...
* Set the access mode;
data catiA; set OnBoard.weta_cati_purp_asuff;
...
data OnBoard.weta_cati_purp_asuff_path; set catiE;
...
data catiA; set OnBoard.weta_cati_purp_asuff_path; 
   ...
   if q400 = 4   then fare_medium = 'ticket (10 booklet)';
   if q400 = 5   then fare_medium = 'ticket (20 booklet)';
   if q400 = 6   then fare_medium = 'ticket (40 booklet)';
   if q400 = 7   then fare_medium = 'pass (monthly)';
   if q400 = 8   then fare_medium = 'pass (day)';
   if q400 = 9   then fare_medium = 'pass (group)';
   if q400 = 10  then fare_medium = 'pass (short hop)';
   if q400 = 11  then fare_medium = 'other';
...
data OnBoard.weta_cati_purp_asuff_path_sd; set catiA;
...
data catiA; set OnBoard.weta_cati_purp_asuff_path_sd;
...
data OnBoard.weta_cati_ready; set catiB;
run;
...
data infoA; set OnBoard.rawWetaInfo;
...
data infoC; set infoB;
  ...
  rename boarding_latitude__y_   = boarding_latitude;
  rename boarding_longitude__x_  = boarding_longitude;
  rename alighting_latitude__y_  = alighting_latitude;
  rename alighting_longitude__x_ = alighting_longitude;

run;

data infoD; set infoC;
...
data OnBoard.weta_info_ready; set infoD; keep id route direction field_language orig_purp_field dest_purp_field survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y last_alighting_x last_alighting_y;
...
data dowA; set OnBoard.rawWetaDowWkday; keep survey_id day_of_the_week;
   rename survey_id = id;

data dowB; set OnBoard.rawWetaDowWkend; keep survey_id day_of_the_week;
   rename survey_id = id;

proc append base = dowA data = dowB force;

data OnBoard.weta_dow_ready; set dowA;
   label day_of_week = "Day of week of field survey";
run;


* Merge the data sets;
data catiA; set OnBoard.weta_cati_ready;
data locA;  set OnBoard.weta_key_loc_taz;
data infoA; set OnBoard.weta_info_ready;
data dowA;  set OnBoard.weta_dow_ready;
...
*                   '12345678901234567890123456789012345678901234567890';
   field_start    = MDY(11,12,2013); 
   field_end      = MDY(11,18,2013);

data OnBoard.weta_ready; set comboC;
run;
