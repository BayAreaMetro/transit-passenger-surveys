...
*          See BuildDatabases.sas for script to create SAS databases.  Specific to Napa Vine.
*          Work in progress.
*
...
%let gis_geocode     = 'M:\Data\OnBoard\Data and Reports\Napa Vine\Geocode\gis_geocode.csv';
%let output_directory = M:\Data\OnBoard\Data and Reports\Napa Vine\_working;
run;
...
* Trip purpose;
data dataA; set OnBoard.rawNapa;
...
data dataC; set dataB; 
  ...
  * If no work and university is present at all, then university;
  if tour_purp = 'missing' and WORK_BEFORE_TRIP = 'No' and WORK_AFTER_TRIP = 'No' and orig_purp = "university" or dest_purp = "university" then tour_purp = "university";

  * if work (school) before then home, assume work (school) tour;
  if tour_purp = 'missing' and WORK_BEFORE_TRIP = 'Yes' and dest_purp = "home"  then tour_purp = "work";
  ...
  * if work (school) after, and home before, assume work (school) tour;
  if tour_purp = 'missing' and WORK_AFTER_TRIP = 'Yes' and orig_purp = "home" then tour_purp = "work";
  ...
  * if no work before or after, but work is a leg, assume work tour (already covered for university);
  if tour_purp = 'missing' and WORK_BEFORE_TRIP = 'No' and WORK_AFTER_TRIP = 'No' and (orig_purp = "work" or dest_purp = "work") then tour_purp = "work";

  * if start at work, no work after, assume a work tour (already covered for university);
  if tour_purp = 'missing' and orig_purp = "work" and WORK_AFTER_TRIP = 'No' then tour_purp = "work";

  * Use the return to work for at work tours, assuming anyone going back to work but not stopping at home is making;
  if tour_purp = 'missing' and WORK_BEFORE_TRIP = 'Yes' and orig_purp^= "home" and dest_purp= "work" then tour_purp = "at work";
  if tour_purp = 'missing' and WORK_AFTER_TRIP = 'Yes' and orig_purp = "work" and dest_purp^= "home" then tour_purp = "at work";
...
data dataD; set dataC;
   ...
   * If you are still left, assume working before or after trip (which does not have a home or work end) puts you on a work tour;
   if tour_purp = 'missing' and (WORK_BEFORE_TRIP = 'Yes' or WORK_AFTER_TRIP = 'Yes') then tour_purp = "work";

   * If you are still left, and one end is home, then other end is purpose;
   ...
   * If you are still left, then orig_purp;
...
data OnBoard.napa_purp; set dataD;
...
data OnBoard.napa_key_loc; set geoB; drop _TYPE_ _FREQ_;
run;
 
data dataA; set OnBoard.napa_purp; keep ID orig_purp dest_purp;
data gisA;  set OnBoard.napa_key_loc; 
...
data OnBoard.napa_key_loc; set gisC; drop orig_purp dest_purp;
run;
...
data dataA; set OnBoard.napa_purp;
...
data dataB; set dataA;
   ...
   workers = .;
   if EMPLYD_IN_HH = 'None' then workers = 0;
   ...
   if EMPLYD_IN_HH = '6 or more' then workers = 6;
...
data OnBoard.napa_purp_asuff; set dataD;
...
data dataA; set OnBoard.napa_purp_asuff;
...
data dataA; set dataA;
   ...
   * strip letter from route variable;
   route = put(ROUTE_SURVEYED_CODE,100.);
...
* Put the Napa Vine transfers in a the other field so all the transfers are in one place;
data dataB; set dataA;

   if (trim(first_prior_raw)  ^= 'OTHER AGENCY ROUTE' and trim(first_prior_raw)  ^= 'OTHER AGENCY') and LENGTH(TRANSFER_FROM_1ST) > 2 then first_prior_raw_other  = first_prior_raw;
   if (trim(second_prior_raw) ^= 'OTHER AGENCY ROUTE' and trim(second_prior_raw) ^= 'OTHER AGENCY') and LENGTH(TRANSFER_FROM_2ND) > 2 then second_prior_raw_other = second_prior_raw;
   if (trim(third_prior_raw)  ^= 'OTHER AGENCY ROUTE' and trim(third_prior_raw)  ^= 'OTHER AGENCY') and LENGTH(TRANSFER_FROM_3RD) > 2 then third_prior_raw_other  = third_prior_raw;

   if (trim(first_after_raw)  ^= 'OTHER AGENCY ROUTE' and trim(first_after_raw)  ^= 'OTHER AGENCY') and LENGTH(TRANSFER_TO_1ST) > 2 then first_after_raw_other  = first_after_raw;
   if (trim(second_after_raw) ^= 'OTHER AGENCY ROUTE' and trim(second_after_raw) ^= 'OTHER AGENCY') and LENGTH(TRANSFER_TO_2ND) > 2 then second_after_raw_other = second_after_raw;
   if (trim(third_after_raw)  ^= 'OTHER AGENCY ROUTE' and trim(third_after_raw)  ^= 'OTHER AGENCY') and LENGTH(TRANSFER_TO_3RD) > 2 then third_after_raw_other  = third_after_raw;

run;

proc freq data = dataB; tables first_prior_raw_other first_after_raw_other;
proc freq data = dataB; tables second_prior_raw_other second_after_raw_other;
proc freq data = dataB; tables third_prior_raw_other third_after_raw_other;
run;
...
data dataG; set dataF;
   ...
   * Access mode;
   ...
   if ACCESS_MODE_CODE = 8 then access_mode = 'knr';
   if ACCESS_MODE_CODE = 9 then access_mode = 'knr';
...
* Compare to ETCs trip_legs_code;
...
data OnBoard.napa_purp_asuff_path; set dataI;
...
data dataA; set OnBoard.napa_purp_asuff_path;
...
data dataA; set dataA; 
   ...
   if PAY_MODE_CODE = 2   then fare_medium = 'pass (24 hour)';
   if PAY_MODE_CODE = 3   then fare_medium = 'pass (20 ride)';
   if PAY_MODE_CODE = 4   then fare_medium = 'pass (monthly)';

   fare_category = put('Missing',36.);
   if FARE_TYPE_CODE = 1 then fare_category = 'adult';
   if FARE_TYPE_CODE = 2 then fare_category = 'senior or disabled';
   if FARE_TYPE_CODE = 3 then fare_category = 'student';
...
data OnBoard.napa_purp_asuff_path_sd; set dataA;
...
data dataA; set OnBoard.napa_purp_asuff_path_sd;
   ...
   return_hour = .;
   return_hour = RETURN_HOME_CODE + 3;
   if RETURN_HOME_CODE = 88 then return_hour = .;
   if RETURN_HOME_CODE = 99 then return_hour = .;
...
data dataB; set dataA;
   ...
   * get day of week and weekpart from the date;
   day_of_the_week = put('missing',10.);
   date_as_date = input(CAT(date,"/14"),mmddyy10.);
   if WEEKDAY(date_as_date) = 1 then day_of_the_week = 'SUNDAY';
   if WEEKDAY(date_as_date) = 2 then day_of_the_week = 'MONDAY';
   if WEEKDAY(date_as_date) = 3 then day_of_the_week = 'TUESDAY';
   if WEEKDAY(date_as_date) = 4 then day_of_the_week = 'WEDNESDAY';
   if WEEKDAY(date_as_date) = 5 then day_of_the_week = 'THURSDAY';
   if WEEKDAY(date_as_date) = 6 then day_of_the_week = 'FRIDAY';
   if WEEKDAY(date_as_date) = 7 then day_of_the_week = 'SATURDAY';

   weekpart = put('Missing',7.);
   if WEEKDAY_OR_WEEKEND = 'Weekend' then weekpart = 'WEEKEND';
   if WEEKDAY_OR_WEEKEND = 'Weekday' then weekpart = 'WEEKDAY';
...
data OnBoard.napa_purp_asuff_path_sd_time; set dataC;
run;
...
data dataA; set OnBoard.napa_purp_asuff_path_sd;
  ...
  survey_boarding_x  = BOARDING_STOP_LON;
  survey_boarding_y  = BOARDING_STOP_LAT;

  survey_alighting_x = ALIGHTING_STOP_LON;
  survey_alighting_y = ALIGHTING_STOP_LAT;
...
data dataC; set OnBoard.napa_purp_asuff_path_sd_time;
...
data locA; set OnBoard.napa_key_loc;
...
data dataE; set dataE;

   operator       = put('Napa Vine',50.);
   field_start    = MDY(5,14,2014); 
   field_end      = MDY(6,21,2014);
...
data OnBoard.napa_ready; set dataE;
run;
