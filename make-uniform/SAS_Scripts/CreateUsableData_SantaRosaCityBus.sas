...
*          See BuildDatabases.sas for script to create SAS databases.  Specific to Santa 
*          Rosa City Bus.
*
...
%let gis_geocode     = 'M:\Data\OnBoard\Data and Reports\Santa Rosa CityBus\Geocode\CATI\gis_geocode.csv';
%let gis_geocode_maz = 'M:\Data\OnBoard\Data and Reports\Santa Rosa CityBus\Geocode\CATI\gis_geocode_maz.csv';
%let output_directory = M:\Data\OnBoard\Data and Reports\Santa Rosa CityBus\_working;
run;
...
data catiA; set OnBoard.rawCityBusCati;
   orig_purp = "                         ";
   dest_purp = "                         ";
...
data OnBoard.srcb_cati_purp; set catiD;
...
data gisA;  set OnBoard.rawCityBusGis;
   rename surveyid = id;
   rename idcode   = qcode;
data catiA; set OnBoard.srcb_cati_purp; keep id q10 q11;
data geoA;  set input;
...
data OnBoard.srcb_key_loc_taz; set geoE; drop _TYPE_ _FREQ_;
run;


* Set the home and work locations if they are an origin or a destionation;
data catiA; set OnBoard.srcb_cati_purp; keep id orig_purp dest_purp;
data gisA; set OnBoard.srcb_key_loc_taz;
...
data OnBoard.srcb_key_loc_taz; set gisC; drop orig_purp dest_purp;
run;
...
data catiA; set OnBoard.srcb_cati_purp;
...
data OnBoard.srcb_cati_purp_asuff; set catiB;
...
* Set the access mode;
data catiA; set OnBoard.srcb_cati_purp_asuff;
...
data OnBoard.srcb_cati_purp_asuff_path; set catiE;
...
data catiA; set OnBoard.srcb_cati_purp_asuff_path; 
   ...
   fare_medium = 'Missing                             ';
   if q218 = 1  then fare_medium = 'cash (bills and coins)';
   if q218 = 2  then fare_medium = 'pass (monthly)';
   if q218 = 3  then fare_medium = 'transfer (santa rosa citybus)';
   if q218 = 4  then fare_medium = 'transfer (not santa rosa citybus)';
   if q218 = 5  then fare_medium = 'pass (citybus ticketbook)';
   if q218 = 6  then fare_medium = 'pass (paratransit id)';
   if q218 = 7 then fare_medium = 'other';

   fare_category = 'Missing                             ';
   if q219 = 1 then fare_category = 'adult';
   if q219 = 2 then fare_category = 'senior';
   if q219 = 3 then fare_category = 'disabled';
   if q219 = 4 then fare_category = 'youth';
...
data OnBoard.srcb_cati_purp_asuff_path_sd; set catiA;
...
data catiA; set OnBoard.srcb_cati_purp_asuff_path_sd;
...
data OnBoard.srcb_cati_ready; set catiB;
run;
...
data infoA; set OnBoard.rawCityBusInfo;
...
data OnBoard.srcb_info_ready; set infoC; keep id route direction field_language orig_purp_field dest_purp_field survey_boarding_x survey_boarding_y survey_alighting_x survey_alighting_y first_boarding_x first_boarding_y last_alighting_x last_alighting_y;
...
data dowA; set OnBoard.rawCityBusDow; keep survey_id day_of_the_week;
   rename survey_id = id;

data OnBoard.srcb_dow_ready; set dowA;
   label day_of_week = "Day of week of field survey";
run;

* Merge the data sets;
data catiA; set OnBoard.srcb_cati_ready;
data locA;  set OnBoard.srcb_key_loc_taz;
data infoA; set OnBoard.srcb_info_ready;
data dowA;  set OnBoard.srcb_dow_ready;
...
* Add the weekpart, operator (make 50 wide), field start, and field end information, geo-coding of the stops was not done for CityBus;
data comboC; set comboB;
   weekpart       = 'WEEKDAY';
   operator       = 'Santa Rosa CityBus                                ';
*                   '12345678901234567890123456789012345678901234567890';
   field_start    = MDY(4,24,2012); 
   field_end      = MDY(4,27,2012);

data OnBoard.srcb_ready; set comboC;
run;
