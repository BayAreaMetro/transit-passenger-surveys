...
EA_WB <- read.csv(EA_West,header = TRUE) %>% select(OTAZ,DTAZ,vol_pax) %>% mutate (period="Early AM", direction="Westbound")  # Early AM, westbound, etc.
...
EA_EB <- read.csv(EA_East,header = TRUE) %>% select(OTAZ,DTAZ,vol_pax) %>% mutate (period="Early AM", direction="Eastbound")  # Early AM, eastbound, etc.
...
# Bind all together, filter by vol_pax > 0, sort file, and write out to CSV
