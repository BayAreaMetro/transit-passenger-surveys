...
TPS_SURVEY_IN = "M:/Data/OnBoard/Data and Reports/_data_Standardized/share_data/model_version/TPS_Model_Version_PopulationSim_Weights2021-09-02.Rdata"
load (TPS_SURVEY_IN)
...
# Now do totals as opposed to operators

survey_income_total <- TPS %>% 
  filter(!(household_income %in% c("under $35,000","Missing","refused","$35,000 or higher"))) %>% 
  filter(!(is.na(household_income))) %>% 
  mutate(
    income_rc=case_when(
...
write.csv(survey_income_total,file.path(Output,"survey_income_total.csv"),row.names = FALSE)
write.csv(survey_race_total,file.path(Output,"survey_race_total.csv"),row.names = FALSE)
