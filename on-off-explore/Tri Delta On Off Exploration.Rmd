...
# Add in an all routes target, scaled up to match the sum of the ETC weights
all_routes_target_df <- data.frame(target_id = c("all_routes"), onoff_count = c(sum(obs_targets_df$onoff_count) * 9849.0 / 7718.9))

obs_targets_df <- rbind(obs_targets_df, all_routes_target_df)

observed_targets_vector <- obs_targets_df$onoff_count

# Step 2: build the importance weights vector (all 1s for now, but experiment with higher weight for regional target)
importance_weights_vector <- observed_targets_vector * 0 + 1
importance_weights_vector[89] <- 500.0
...
# Step 5: run the optimization (generically and with lower and upper bounds)
...
optimal_weights_test <- optimx(starting_weights_vector, 
                               fn = optimization_function, 
                               method = "L-BFGS-B",
                               lower = 1.00,
                               upper = 400.00,
...
compare_weights <- left_join(tablet_etc_weights, tablet_weights, by = c("route", "direction", "time_period"))

sum(compare_weights$sum_weights)
sum(compare_weights$optimal_weights)
